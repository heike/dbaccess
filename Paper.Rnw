...
The standard and random binning algorithms are easily extendible to higher dimension. For the purposes of creating binned scatterplots we will specify extension to rectangular binning in two dimensions. In this case we wish to assign data tuples $(x_i, y_i)$ to bin centers of the form $(x_j^\ast,y_j^\ast)$, with $1 \le j \le N_X \cdot N_Y =J$, where $J$ total number of indexed bins in $\mathbb{R}^2$. The standard rectangular binning function $b(.) : (x_i,y_i) \rightarrow (x^\ast_j,y^\ast_j)$ is defined as 
%
...
\end{eqnarray}
%
where $b^r_X(x_i)$ and $b^r_Y(y_i)$ are univariate random binning algorithms for each dimension.
...
%   {./figure/RandomBinningDataPoints1}}
  ...
  \caption{Points are assigned to bins centered at (5, 25) an (15,25). Points in the standard algorithm are contained within the defined bin region, where points from the random algorithm may be assigned to bins from outside the defined bin regions.}
