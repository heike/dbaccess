...
\item {\it Frequency Loss}, $L^\text{F}$, occurs when frequency information is not rendered accurately. If a discrete color scale is used to display bin counts, then entire ranges of counts will be mapped to the same color; thus lossing precision in the display of frequencies.
\end{itemize}
...
\end{eqnarray}  
%
\ktm{Visual display of spatial loss still needed.}

\subsection{Frequency Loss}

\ktm{If the shading scheme for rendering the counts in each bin is discretized there is a loss of frequency information. While counts can certainly be modeled as continuous on the HSV color space (reference needed) discrete color breaks may be prefered for a few reasons. %The average person is able to  On the continuous scale, a viewer will likely not be able to exactly match a tile shading to the corresponding count on the color key in the legend of the plot without an aid such as interactive mouseover effects. 
}
