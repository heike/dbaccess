...
For the bivariately skewed exponential data, the one unit origin offset minimizes the net loss for the smaller intervals but was not optimal for the largest intervals; 2.5\% and 7\% increase the minimum spatial loss for bins of size eight and ten, respectively.
...
Spatial binning reduces data to contain the center and frequency information for all bins.
While bin frequencies could be mapped continuously to a precisely rendered shade, it is naive to believe that this information can be extracted at the same resolution due to limitations of our perception. 
Implicitly, we lose information  when (imperfectly) mapping the shade of a tile to its frequency. In order to make this loss explicit, we introduce a second stage of binning and discretize the color scale for frequencies in the spatially binned scatterplot.

Figure~\ref{fig:StandardFreqBinning4710none} displays several spatially binned scatterplots. On the right frequency is mapped continuously to color; on the left  there is a varying numbers of  standard binned frequencies. It is trivial to discern differences between similarly shaded tiles
when there are only four shades. This becomes more difficult -- but still possible -- at seven frequency bins, but at ten frequency bins we are hardly able to discriminate between colors better than in the continuous shading. 
This aligns with Healey and Enn's theory on the number of discernible colors \citep{colorperception}. We will therefore use an upper limit of ten frequency bins in our exploration of frequency loss.
...
The loss of frequency information in frequency binning is dependent on the selection of a discrete color mapping, where the binning algorithm and number of frequency bins must be specified. The top row in Figure~\ref{fig:FreqLossCompare} displays the frequency loss from using standard and quantile frequency binning algorithms, with between one and ten frequency bins, from each set of simulated data. 

There is a huge difference in the magnitude of frequency loss between the distributions: frequency losses are the highest in the scatterplot of exponentially distributed data, much lower for the normally distributed data and lowest in the case of the uniform data.
For all distributions and both binning algorithms, frequency loss decreases as the number of frequency bins increases. This initially sharp decrease  largely flattens out at about four bins.
Thus four and seven bins for color shades allow us to reduce frequency loss while allowing for easy distinction between frequency groups in the binned scatterplot.

Lastly, for small number of bins, the frequency loss from standard frequency binning is higher than loss from quantile-based binning. However, in all three simulated data sets, the losses drop to similar levels for both algorithms when four or more frequency bins are used. 

The bottom row of Figure~\ref{fig:FreqLossCompare} displays the log frequency loss from using standard and quantile algorithms for binning the \textit{log} bin counts from the same sets of simulated data. 
Frequency binning using log counts exhibits a very similar behavior to binning of untransformed frequencies.
The difference in scales makes it impossible to directly compare frequency loss and log frequency loss. 
But the same advice to use between four and seven frequency bins also holds for shading a binned scatterplot based on log counts. 
...
Log transforming the frequencies prior to binning and using quantile based binning on the raw frequencies are two methods for dealing with the same problem in binned scatterplots: the distribution of bin frequencies is heavily right skewed. High frequency bins visually overshadow any structure in low frequency bins. 

Frequency groups for quantile binning are invariant to the log transformation because a monotone transformation does not affect groupings based on quantiles, thus it is preferable leave the frequencies untransformed before quantile binning for better contextual interpretability.
In the common situation where bin frequencies are heavily skewed, the choice is between quantile frequency binning and standard log frequency binning.

As losses from log-transformed and un-transformed frequencies are not directly comparable, the choice is guided by desired interpretation. 
For standard log frequency binning, the shade is to be interpreted as an ordinal indicator of the groups based on equally spaced groupings of log bin frequencies, whereas for quantile frequency binning the shade denotes groups based on frequency quantiles. This is analogous to the difference in interpreting a histogram of log transformed data and a boxplot in univariate visualization.

Figure~\ref{freqbinning} shows binned scatterplots for the simulated normal data with quantile frequency binning and standard log frequency binning. \ktm{Both shading schemes are effective at reducing the visual impact of the highest density at the center, allowing for the differences in the surrounding bin frequencies to be emphasized. It should be noted that the frequency binning algorithm produced binned scatterplots that visually very similar, but this will not always be the case. It occured in this scenario because the distribution of log bin frequencies from the bivariate normal data is nearly uniform, thus the standard log binning aligns closely with the quantile binning.}
...
For this data example we revisit the baseball data used earlier. To construct the binned scatterplot to display the relationship between strikeouts and game counts, we begin by specifying the bin dimensions and origin for standard rectangular binning. The data are counts, each variable is therefore recorded as an integer, corresponding to a resolution of one. We use integer bin widths in each dimension of spatial bins  to avoid artificial density stripes. The data has only 42,583 observations, so we can bin finely without taking an inordinate amount of computation time. We use approximately 50 bins in each dimension; the data range from 1 to 106 games and 0 to 513 strike-outs, thus we use bins that are two games wide and ten strikeouts high. The origin will be set to a half unit below the minimum value in each dimension, at $(0.5,-0.5)$. 
 

The leftmost plot in Figure~\ref{fig:BaseballBinnedScatters2X10} displays the binned scatterplot with a continuous shading of raw bin frequencies. The most striking feature in the frequency distribution is the dark spot in the bottom right of the plot representing a large number of pitchers that played very few games and had very few strikeouts. It is nearly impossible to differentiate the density structure across the majority of bins representing better pitchers who played many games and earned many strikeouts. 
...
\label{proof:offset}
\hh{you need to use the same conditions for the proof as you are stating when you refer to it. A uniform distribution was not mentioned before. Also, make sure that this section is 'stand-alone', i.e. explain every single one of the parameters that you are using.}
