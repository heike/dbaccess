...
\ktm{  Paragraph: Discussion of types of deterministic rectangular binning.  Standard and Quantile. Table~\ref{tab:rectbinning} } 
...
%
\ktm{  for $x_i \in [x^\ast_{j+1}, x^\ast_{j}]$.  This probability structure is inversely proportional to the distance from each bin center. This method is easily extensible to also map $x_i$ into more than two bins and can accommodate non-uniform bin sizes. }
