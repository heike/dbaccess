...
\newcommand{\svp}[1]{{\color{red}#1}} %green: Susan (It's more fun that way - comment roulette)


%opening
\title{Binning Strategies and Related Loss for Graphics of Large Data}
\author{Susan VanderPlas$^1$, Karsten Maurer$^1$, Heike Hofmann$^{1,2}$\\$^1$Department of Statistics, $^2$Human Computer Interaction\\Iowa State University}

\begin{document}
% \SweaveOpts{concordance=TRUE, warning=FALSE,message=FALSE}
\maketitle

\begin{abstract}
Dealing with the data deluge of the Big Data Age is both exciting and challenging. Large data demands require us to re-think strategies of visualizing data. Binning methods have been suggested in the past as viable alternative to standard plots based on raw data, as the resulting area plots tend to not be affected by increases in data as much. This comes with the price of loss of information inherent to any binning scheme. In this paper we discuss properties of two commonly used binning algorithms. We define loss of information in the specific setting of two dimensional displays, provide readily applicable tools for loss evaluation and discuss the two binning schemes with respect to their loss in the frame work of a simulation and two case studies.
\end{abstract}
...
opts_chunk$set(concordance=TRUE, tidy=TRUE, cache=TRUE, message=FALSE, warning=FALSE)
...
d1.ext <- xtd.data(d1)
@
<<scatterplots, dependson='setup', echo=F, include=FALSE>>=
library(hexbin)
qplot(data=d1.ext, x=G, y=SO, geom="point", main="Games and Strike Outs in ML Baseball", alpha=I(.05), size=I(3)) + theme_bw()
  ggsave("./figure/OverplottingAlpha.pdf", width=5, height=5)
qplot(data=d1, x=G, y=SO, geom="point", main="Games and Strike Outs in ML Baseball", size=I(3)) + theme_bw()
  ggsave("./figure/Overplotting.pdf", width=5, height=5)
qplot(data=d1, x=G, y=SO, geom="point", main="Games and Strike Outs in ML Baseball", shape="o", size=I(6)) + scale_shape_identity() + theme_bw()
  ggsave("./figure/OverplottingCircles.pdf", width=5, height=5)
qplot(data=d1.ext, x=G, y=SO, geom="hex", fill=..count.., group=1) +  
  scale_fill_gradient("Frequency", low="#56B1F7", high="#132B43", guide="legend", trans="log", breaks=c(1, 10, 100, 1000)) + theme_bw() + theme(legend.position="bottom")
...
qplot(data=d1.hex, x=G, y=SO, geom="point", size=Freq) + 
  scale_size_area("Frequency", max_size=4, trans="log", breaks=c(1, 10, 100, 1000, 10000))+ theme_bw() +
...
\begin{quote}``It’s a revolution ... The march of quantification, made possible by enormous new sources of data, will sweep through academia, business and government. There is no area that is going to be untouched"\end{quote} (Gary King, Harvard Institute).  
...
\citet{Friedman97} proposed in his paper on datamining and statistics that ``Every time the amount of data increases by a factor of ten, we should totally rethink how we analyze it". The same holds for visualizations.  
With a 100-1000 fold increase in the amount of data, the utility of some of  graphs changes dramatically, with some of our most-used tools such as the scatterplot deteriorating quickly \citep{gold}. 

% binning as a way out
Area plots, such as histograms, do not tend to be as affected by increases in the amount of data as plots, which utilize  raw data. 
This fact is being exploited to make scatterplots again useful instruments for large data settings  by employing binning strategies \citep{gold}.
...
\begin{figure}[hbtp]
  \subfloat[Overplotted data ]{\includegraphics[keepaspectratio=TRUE,width=.31\textwidth]{figure/Overplotting.pdf}}
  \subfloat[Tukey-style open circles]{\includegraphics[keepaspectratio=TRUE,width=.31\textwidth]{figure/OverplottingCircles.pdf}} 
	\subfloat[Alpha blending]{\includegraphics[keepaspectratio=TRUE,width=.31\textwidth]{figure/OverplottingAlpha.pdf}}
  
  \subfloat[Hexagonal binning]{\includegraphics[keepaspectratio=TRUE,width=.48\textwidth]{figure/HexBinning.pdf}}
  \subfloat[Scatterplot of Binned Data]{\includegraphics[keepaspectratio=TRUE,width=.48\textwidth]{figure/BinScatter.pdf}} 

	\caption{Scatterplots of Games versus Strikeouts in Major League Baseball, using different strategies of dealing with the issue of over-plotting:  (a) uses standard, opaque, filled circles, (b)  uses Tukey's recommended open circles, and (c) uses filled circles with alpha-blending ($\alpha$=0.05). Plots (d) and (e) show hexagonal binning strategies with frequency mapped to color and area respectively}
...
Histograms are an example of plots which use binned variables. To extend histograms to a graph which can represent the joint distribution between two variables, it is a natural step to create a two-dimensional graph which uses some other attribute (color, 3D rendering) to provide joint density information. A {\it tile plot} uses shading to provide frequency information, with tiles (rather than bars in a histogram) centered at the bin center, rather like a two-dimensional histogram viewed from above. 
...
When the point area is scaled in proportion to frequency, scatterplots can be used to display the binned data as well.  When points are filled circles, these plots are also known as ``bubble plots", which were first used by William Playfair \citep{playfair, playfair2}. Kernel density smoothing can be used to vary $\alpha$ or color according to a smoothed density, providing features similar to tile plots or $\alpha$-blended scatterplots in a more smooth, continuous fashion. These estimates require parameter tuning and may hide gaps in the data by over-smoothing while simultaneously de-emphasizing outlying points.

As alternatives, Figure~\ref{fig:scatter-alpha} (d-e) contains examples of a hexagonal tile plot with frequency encoded as color (d) and a ``bubble plot" (referred to as a modified scatterplot) with frequency encoded as point size (e). The hexagonal tiles and the modified scatterplot are more effective at displaying the shape of the joint density and preserving outliers than any of the scatterplots shown in Figure~\ref{fig:scatter-alpha} (a-c). The tile plot is less prone to the Hermann-grid illusion than the bubble plot, particularly for binned data, where bin centers usually fall on a grid.
...
% \end{figure}
Only in the tile and the bubble plot the inner structure of the data becomes apparent: 
...
For extremely large data sets, tile plots are a more useful visualization of two-dimensional density information than the scatterplot, and are less computationally demanding, as not every single point in the data set has to be  rendered separately. 
...
We will only consider binning in two dimensions, $X$ and $Y$. The algorithms we discuss are immediately applicable to higher dimension, but we do not feel that the paper would benefit from a more general discussion.
...
Let $x_{(i)}$ correspond to the ordered values of $X$, where $i=1, ..., n$ and $n$ is the number of total observations. Then $x_{(1)}$ and $x_{(n)}$ then denote the observed minimum and the maximum of $X$, respectively. 

We then define a set of bin centers $\{ x_j^\ast \}$, with $1 \le j \le n_X$, where $n_X$ is the overall number of bins in dimension $X$. Binning can either be determined by the number of bins, $n_X$, or, equivalently, by the bin width, $b_X$. For standard binning the relationship between these two parameters is  given as 
\[
b_X = (x_{(n)} - x_{(1)})/n_X
\]

Mathematically, binning corresponds to a function $b(.)$ that assigns to a value $x$ the closest bin center $x^\ast \in \{ x_j^\ast \}, j \in \{1, ..., n_X\}$, i.e. the binning function   $b(.) : x \rightarrow x^\ast$ is defined as
\begin{eqnarray*}\label{stdbin}
x^\ast = b(x) =  b_X \cdot \left\lfloor (x-O_X)/ b_X + b_X/2\right\rfloor + O_X
\end{eqnarray*}
where $O_X$ is the center of the first bin and $\lfloor . \rfloor$ denotes the floor function, $\lfloor x \rfloor$ is the largest integer value smaller than $x$. 
%to allow for bins of width $b_X$ centered at any sequence $O_X, O_X + b_X, ..., O_X + n b_X$. 

For illustration, let us assume that we have observed values 1.23, 1.55, and 2.35, and bin centers $\{1, 2, 3\}$, corresponding to a bin width of $b_X = 1$ and a first bin center $O_X = 1$.
Intuitively it is clear, that value 1.23 is assigned to bin center 1, while values 1.55 and 2.35 are assigned to bin center 2.
Applying equation (\ref{stdbin}) to value 1.23 we get: $1 \cdot \lfloor (1.23-1)/ 1 + 0.5\rfloor + 1$ = $\lfloor 0.73 \rfloor + 1$ =  0 + 1 = 1. Similarly, 1.55 is assigned to $\lfloor 0.55 + 0.5\rfloor + 1$ = 1+1 = 2.
...
% , with smallest bin
% $b_X   \cdot \lfloor (x_{(1)}-O_X)/b_X + 0.5\rfloor + O_X$ and largest bin 
% $b_X   \cdot \lfloor (x_{(n)}-O_X)/b_X + 0.5\rfloor + O_X$. 

Figure~\ref{fig:BinningTutorial} provides an illustration of the binning process.
<<binningDemo, echo=FALSE, include=FALSE>>=
      library(mvtnorm)
      set.seed(33)
...
#                  label=expression(paste('(', over(phantom(0),x)[i], ',', over(phantom(0),y)[i],')'))) +
      ...
      #ggsave("./figure/Binning-DataVisualNumericalCenter.pdf", width=5.5, height=3.25, units="in")
...
\begin{figure}[hbtp]
  \centering
  \subfloat[Standard Algorithm]{\includegraphics[keepaspectratio=true, width=3in]{./figure/binningDemo1.pdf}}
...
%   {./figure/RandomBinningDataPoints1}}
  ...
  {./figure/RandomBinDemo.pdf}}
  \caption{Points are assigned to bin (0, 20) and bin (10,20). Points in the standard algorithm are all contained within the bin region, where points in the random algorithm may be outside the direct region.}
  \label{fig:BinningTutorial}
\end{figure}

\subsection{Random Binning Algorithm}
<<RandomBinDemo,echo=F,include=FALSE,cache=TRUE>>=
nl <- ncol(d.rand)
dnew <- d.rand
binning <- c(10, 10)
for(x in 1:(nl)){
...
This method is easily extensible to non-uniform bin sizes and can also map $x$ to more than two bins, unlike the standard algorithm. 
...
\begin{itemize}
\item {\it Numerical Loss} occurs when the data set is reduced  to bin centers, i.e. this is the difference between a data point $(x,y)$ and the corresponding bin center $(x^\ast, y^\ast)$.

\item {\it Frequency Loss}  results from our inability to render the frequency information accurately. Different rendering methods for the frequency information will lead to different losses. For the remainder of the paper we will assume that we are using colors in tile plots to represent this information. The number of colors that can be differentiated and mapped back to frequency information accurately is fairly limited. Note that even though this loss will turn out to be substantial, it is, in fact, a huge gain with respect to the original scatterplot, where there is no frequency information, so that points which are over-plotted are not at all distinguishable.

% 
% , so we must balance perceptual limits of frequency information with the limits of overplotting single points. 

\item {\it Visual Loss}: While the visual loss is most salient, it is an implicit loss due to the rendering;  visually we perceive the center of a bin to be the center of the tile. This will generally different from the center of the data corresponding to that bin as given by the mean. This causes some bias in the perception.
...
<<tablelossexample, echo=FALSE, results='asis',cache=FALSE>>=
...
We explore the different types of loss in the remainder of this section in more detail.

\subsection{Loss in Spatial Dimensions}
Reducing data points $x$ to bin centers $\hat x$ results in a loss of information, as there is a reduction in the variability of the data when data points are transformed from individual values to bin centers on a grid. Let us denote the difference between point $x_i$ from its bin center as $S_i = \left|x_i-b(x_i)\right|$ for each point in the data set $i=1, ..., N$. The total loss from binning an entire data set is defined as 

\[L_s(x) = 1/S^2_{\emptyset} \cdot \sum_{i=1}^{N} S^2_i\] 
where $S_\emptyset^2$ is the loss which results when the entire data set is reduced to a single bin,
$S^2_{\emptyset} = \sum_i (x_i-\tilde{x})^2$ for bin center $\tilde{x}$.
...
If we consider the bin center to be the numerical mean, then the loss is the sum of the variance of the data in each bin, compared to the variance of the data set as a whole, and $L_s(x) \in [0,1]$. 

The disadvantage to this choice is that this introduces  visual bias  to the graphical representation of the data, as we perceive the bin center to be the physical middle of he symbol representing the data.
Therefore we distinguish two contributions to this loss:
Numerical loss ($L^\text{N}$) results from the binning process as points are reduced to the numerical mean center, and visual loss ($L^\text{V}$) is due to the difference between the mean of points within the bin and the visual center of the bin. This discrepancy is particularly clear as demonstrated in Figure~\ref{fig:stdBinTutorial2}, in which the visual center of the bin varies significantly from the numerical center of the data contained within the bin under the standard algorithm.
...
L_s(X) =  1/S^2_{\emptyset} \cdot \left(\sum_i L^N_i + \sum_i L^V_i\right)
\end{eqnarray}

<<smoothing, echo=FALSE, include=FALSE, eval=FALSE>>=
library(plyr)
library(ggplot2)
library(reshape)
set.seed(53022)
...
ggplot()+
  geom_histogram(aes(x=data$x, y=..count..), binwidth=1/4, fill=I("grey80"), colour=I("grey80"), alpha=I(.5)) + 
  geom_histogram(aes(x=data$x, y=..count../4), binwidth=1, fill=NA, colour=I("grey50"), alpha=I(.5)) + 
  theme_bw() + xlab("x") + ylab("Frequency") + ggtitle("Frequency Loss")
...
\paragraph{Loss in the Random Assignment Algorithm:}
An interesting feature of the random binning algorithm is that the loss that we experience by doubling the bin width is deterministic, i.e. independent even though the assignment of bins is not. This holds for repeated doubling of the bin width. A proof of this can be found in appendix \ref{proof:constantloss}. Doubling bin widths is, particularly in the framework of visualizations, a very natural step, making this an important property of the random binning.

The loss as a function of bin width for both algorithms is shown in Figure~\ref{fig:LossComparison1}.

\begin{figure}[hbtp]
    \centering
		\subfloat[Spatial Loss]{\includegraphics[keepaspectratio=true, width=.54\textwidth]{binningDemo2}}
    \subfloat[Frequency Loss]{\includegraphics[keepaspectratio=true, width=.45\textwidth]{./figure/FreqLoss.pdf}}
\caption{\label{fig:stdBinTutorial2} Types of loss due to binning: (a) The visual center of the bin is different from the numerical center of the bin, leading to two different sources of loss.  (b) Frequency loss associated with a change of bin width to 1 from 0.25 in the example of Figure~\ref{binning}.}   
	\end{figure}

\subsection{Loss of Frequency Information} 
Loss of frequency information happens because of our inability to render elements of a third dimension efficiently. By employing a color scheme for filling tiles according to the observed frequencies corresponds to a binning of the frequencies into a few different levels, with losses stemming from the same source as the previously discussed numerical loss.
We will assume that we have a set of size $n_C$ of  frequency levels $\{c^\ast_j\}$ with $1 \le j \le n_C$, and that an observed  bin frequency $c$ is assigned to the closest frequency level $c^\ast = b(c)$.  
The --unscaled-- frequency loss is then defined as
\begin{eqnarray}\label{eqn:frequency}
L_\text{Freq} = \sum_{i=1}^{n_X n_Y} \left(c_i - b(c_i)\right)^2
\end{eqnarray}
...
L_{\log \text{Freq}} = \sum_{i=1}^{n_X n_Y} \left(\log(c_i+1) - \log(b(c_i)+1)\right)^2
\end{eqnarray}
...
To examine this further, we simulated points from three different distributions: Uniform, Normal, and Exponential to determine under what circumstances log frequency scaling is performing better than linear frequency scaling. Figure~\ref{fig:logcolor} gives an overview of this simulation. In the top row tile plots with a color scheme based on linear frequency scaling is used, the bottom row shows the same data based on log frequency scaled colors.
...
\begin{figure}[hbtp]
\centering
<<FrequencyCategories, echo=FALSE, out.width='\\linewidth', fig.width=8.5, fig.height=11>>=
...
# linear binning
# pdf(file="figure/p2.pdf", width=8, height=3)
# p2
# dev.off()
...
@
%\includegraphics[width=\textwidth]{figure/MultipleDensityFigure}
...
\end{tabular}}\end{minipage}\hfil
\begin{minipage}[t]{.33\textwidth}\centering\subfloat[Binned Data, 49 rows]{
...
\begin{minipage}[t]{.33\textwidth}\centering\subfloat[Recovered Data, 49 rows]{
...
\caption{\label{RecoveryComparison}Six rows from Original, Binned, and Recovered Data Tables, with data storage sizes}
\end{table}
...
\subsection{Comparison of Binning Algorithms}
<<LossTableGraphs,echo=F,results="hide",cache=TRUE,fig.keep='none', eval=FALSE>>=
# connect <- dbConnect(dbDriver("MySQL"), user="2009Expo", 
#                    password="R R0cks", port=3306, dbname="baseball", 
#                    host="headnode.stat.iastate.edu")
# pitch <- new("dataDB", co=connect, table="Pitching")
...
computeLoss <- function(binning, data){
  ...
  datplot <- rbind(cbind(lRandom$NewData, Algorithm="Random"), cbind(lMean$NewData, Algorithm="Standard"))
  datplot <- ddply(datplot, .(G, SO, Algorithm), summarise, Freq=sum(Freq))
  qplot(G, SO, fill = Freq, geom="tile", data = datplot, 
      main=paste("Baseball: Binning by (", binning[1], ", ", binning[2],")", sep="")) + 
    facet_grid(.~Algorithm) +
    scale_fill_gradient(low="#56B1F7", high="#132B43", guide="legend", trans="log", 
                        breaks=c(1, 5, 50, 500, 5000)) + 
    theme_bw() + theme(legend.position="bottom")
  ggsave(paste("./figure/RandomStd", binning[1], binning[2], ".pdf", sep=""), 
         width=8, height=5, units="in", dpi=1200)

  return(rbind(
    data.frame(binX = binning[1], binY = binning[2], Function = "Random", loss=lRandom$Loss[,5:7]), 
    data.frame(binX = binning[1], binY = binning[2], Function = "Standard", loss=lMean$Loss[,5:7])))
}
...
At the example of the pitching data, we will %consider the variables Games and Strikeouts, which are strongly correlated. This example 
highlight an important advantage of random binning. The two tile plots at the top of Figure~\ref{fig:binning-comparison} show bins of the same width, but clear differences between the binning results are evident: while the standard binning algorithm has rounding artifacts that are clearly visible for even bin widths, the random algorithm produces a graph that better represents the true shape of the data.
\begin{figure}[hbtp]
\centering
\subfloat[(2, 2) tiling of G, SO data]{
\includegraphics[keepaspectratio=true, width=.9\textwidth]{images/RandomStd210.pdf}
}\\
\subfloat[(5, 25) tiling of G, SO data]{
\includegraphics[keepaspectratio=true, width=.9\textwidth]{images/RandomStd525.pdf}
}
\caption{\label{fig:binning-comparison} Comparison of random and standard binning: standard binning introduces artificial striping when the chose bin width is not a multiple of the data resolution.}
\end{figure}

In contrast, when the bin widths are odd, the two binning algorithms yield nearly indistinguishable tile plots, as shown in the bottom row of Figure~\ref{fig:binning-comparison}.

The spatial loss is a result of our desire to reduce and summarize the data until it is more manageable in size, and hence is a factor in our decision to bin the data in the first place. More importantly, frequency loss has a larger effect on the utility of the graphical data summary. Figure~\ref{fig:binning-comparison} shows clearly that the standard algorithm can produce misleading graphical summaries. In order to quantify the advantages of each algorithm, we compare the spatial and frequency loss at different bin widths in $x$ (Games). The random algorithm has higher spatial loss than the standard algorithm, but in many cases the tile graphs are visually indistinguishable. The numerical loss is a function of the distribution and bin width, and once the bin width is chosen, it is entirely fixed. As the visual loss is the difference between the numerical center of points in the bin and the geometric center of the bin, it is a function of the distribution and bin width (and the specific random assignment, in the case of the random algorithm). We would not expect to notice the difference in the spatial loss in either algorithm, so long as the bin width is constant. The far more important factor is the frequency loss, as we perceive that loss in the variability of the shading of bins. 

<<LossComparison1, echo=FALSE, eval=T, fig.cap='Spatial Loss increases with bin width for each variable. The random algorithm has higher spatial loss than the standard algorithm.', fig.width=6, fig.height=4, out.width='.75\\linewidth', out.height='.495\\linewidth', fig.align='center', fig.pos='H'>>=
...
losses <- read.csv("../dbData/data/losses.csv", stringsAsFactors=FALSE)

qplot(x=x, y=TotalLoss.G*100, group=interaction(y, Algorithm), data=subset(losses, y<=10), geom="line", colour=Algorithm, ylab="Percent Spatial Loss", xlab="Bin  Width  (Games)")+ theme_bw() + theme(legend.position="bottom") + ggtitle("Spatial Loss:  Baseball Games") + guides(colour = guide_legend(override.aes = list(size = 2))) + facet_wrap(~Algorithm)
# qplot(x=y, y=VisLoss.SO*100, group=interaction(x, Algorithm), data=subset(losses, x==1), geom="line", colour=Algorithm, ylab="Percent Visual Loss", xlab="Bin Width (Strike Outs)") + theme_bw() + theme(legend.position="bottom") + ggtitle("Visual  Loss  with  increasing  Bin  Width: Baseball Strike Outs\n Games not binned") + guides(colour = guide_legend(override.aes = list(size = 2)))
@
The standard algorithm is preferable because it has lower spatial loss, but the two algorithms are very similar when we compare frequency loss. The random algorithm does have more variability, but the frequency loss values are much more similar, and when bins are of size two in either or both dimensions, the random algorithm has lower frequency loss than the standard algorithm. This holds for bins which are powers of two, as long as the random binning algorithm is applied iteratively to take advantage of the constant loss property noted above.  
<<LossComparison, echo=FALSE, fig.cap="Frequency Loss increases as bin width increases in Games and Strike Outs. The random binning algorithm has slightly higher frequency loss as bin width grows large, but the loss is comparable when bin widths are small.", fig.width=8, fig.height=4, out.width='\\linewidth', out.height='.5\\linewidth', fig.pos='H'>>=
# 
# connect <- dbConnect(dbDriver("MySQL"), user="2009Expo", 
#                      password="R R0cks", port=3306, dbname="baseball", 
#                      host="headnode.stat.iastate.edu")
# pitch <- new("dataDB", co=connect, table="Pitching")
# d1 <- dbData(pitch, vars=c( "G", "SO"))
# bins <- expand.grid(x=c(1:10, 15, 20), y=c(1:5, seq(10, 100, 15)))
# 
# library(multicore)
# lossesRdm <- do.call("rbind", mclapply(1:nrow(bins), function(i) lossCalc(data=d1, binning=c(bins[i,1], bins[i,2]), type="random")))
# lossesStd <- do.call("rbind", mclapply(1:nrow(bins), function(i) lossCalc(data=d1, binning=c(bins[i,1], bins[i,2]), type="standard")))
...
qplot(x=y, y=TotalLoss.LogFreq, group=interaction(x, Algorithm), data=losses, geom="line", colour=Algorithm, ylab="Frequency Loss", xlab="Bin Width (Strike Outs)")+ facet_grid(.~Algorithm) + scale_colour_discrete(guide="none") + theme_bw() + ggtitle("Frequency Loss: Baseball Games and Strike-Outs")
@
...
@
\section{Conclusion}
Large Data sets of continuous variables are very difficult to visualize in raw form, due to over-plotting of points. Binning allows for the visualization and manipulation of large data sets, and easily translates into tile plots which are more appropriate for the human visual system. The loss due to binning reduces the data set to a more manageable size, but some of that loss can be recovered with additional computational investment.
...
<<PrintTable,echo=F,include=FALSE>>=
