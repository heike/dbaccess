...
Figure~\ref{fig:OriginOffsetSimulations} shows how the net spatial loss changes as the origin is slide from the minimum value to one binwidth below the minimum value using the coarse simulated data with a resolution of 2 units. 
For the two symmetrically distributed data sets, normal and uniform, the origin offset of 1 unit below the minimum data value was found to minimize the net spatial loss for each considered bin size (2, 4, 6, 8, and 10 units). \hh{XXX - no! the value at -1 is close, but the dots are in the actual minimal values. This is only a single example - the expected minimum might still be in -1. }
