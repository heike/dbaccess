...
We could also consider a quantile based binning algorithm where bin widths are adjusted such that each bin contains an equal proportion of the observed values. This is a fundimentally different approach than the standard and random binning algorithms defined previously where the goal was to display approximate density. This binning will not be considered for binning the data directly, but instead for further binning the initial bin counts for the purposes of rendering colors on a discrete color palette. 
...
\end{eqnarray}
%
\noindent and $\left\lceil \cdot \right\rceil$ is the ceiling function that rounds any real number up to the next highest integer.  To better understand this function we notice that $\left\lceil J/N_c \right\rceil$ represents the number of values to be assigned to each quantile bin, $n_q$, rounded up; thus the function assigns the $j^{th}$ value to the next lower ordered value that is a multiple of $n_q$.
