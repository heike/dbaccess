...
For this data example we will revisit the baseball data used earlier in this paper.  The characteristics of loss described in the discussion above are exemplified by viewing the loss for the baseball data in Figure ~\ref{fig:LossComparison1}.
...
lossmelt <- lossmelt[which(lossmelt$y <= 10),]
...
qplot(x=x, y=value,
      ...
      xlab="Bin  Width  (Games)", ylab="Proportion Loss")+   
        ...
        ggtitle("Loss for Baseball Data under Standard and Random Binning")


@
\caption{\label{fig:LossComparison1} Loss funcion }
\end{figure}
