...
\ktm{ A number of scatterplot adaptations have been introduced that avoid overplotting by manipulating point locations or location scales. \hh{These approaches do not help in the situation of ties in the data, which are very common for large data sets due to the finite resolution of data collection (e.g.\ airline data is collected at minute intervals).}  Generalized scatterplots \citep{Keim2010GenScatter} display all individual observations but use distortion of the point locations to avoid overlapping points. A\hh{n} extension of generalized scatterplots uses clustering and local principal components to allow ellipsoid oriented distortion to display local correlation structure in the data \citep{Janetzko2013Ellipse}. Variable binned scatterplots \citep{Hao2010visual} break the display into a non-uniform rectangular grid and resize the rows of cells according to density of points. This variable binning fragments the continuity of the axes into many segments on different scales. Theses approachs make fine data structure more visible and allows color to be reserved for a third variable instead of frequency; however, the distortion of the point locations or the axes wrap the visual display of the association between the two primary variables.} \hh{please check the spelling in the previous paragraph. }
...
\hh{XXX Motivation first - starting with the limitiations too  negative.}
...
\ktm{Binning is a two-step procedure: we first assign each observation $(x, y)$ to a bin center $(x^\ast,y^\ast)$, and in a second step we count the number of observations assigned to each unique bin center; resulting in  triples  of the form $(x^\ast, y^\ast, c)$, where $c$ is the number of all observations assigned to bin center $(x^\ast,y^\ast)$.}


 We will proceed with rectangular bins for simplicity, but other binning schemes, such as hexagonal bins \citep{scatterplots} are also common. Rectangular bins are advantageous because bins in $x$ and $y$ are orthogonal to each other, thus, we can present the one-dimensional case which will easily generalize to two or more dimensions.

\hh{There are a lot of commonalities between the different univariate binning algorithms - it would make sense to write a single, more general section, and give a list of specific parameterisations for the individual binning approaches. }
...
\hh{The relationship between value pair $(x_i, y_i)$ and bin center $(x^\ast_j,y^\ast_j)$ needs to be discussed further - because it is not the same as for the univariate situation. $J$ is defined, as above as a multiple of $N_X$ and $N_Y$, this needs to be properly adjusted in the binning algorithm. It will be some linear relationship of the form $j_X\cdot N_X + j_Y$. }  \ktm{ I am having trouble tring to reconcile the indexing of bins in 2 dimensions.  If we use an pair of indeces it makes the extension of binning to 2 dimensions easy; but it makes the notation messier and loss functions more difficult to define. }
