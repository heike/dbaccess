...
% <<options-knitr,echo=F,include=F>>=
% require(knitr)
% options(replace.assign=TRUE,width=70)
% opts_chunk$set(fig.path='figure/graphics-', cache.path='cache/cache-', fig.align='center', fig.width=5, fig.height=5, fig.show='hold', cache=TRUE, par=TRUE)
% @
...
\begin{figure}[hbtp]
  ...
  \subfloat[Hexagonal binning]{\includegraphics[keepaspectratio=TRUE,width=.48\textwidth]{HexBinning.pdf}}
  \subfloat[Scatterplot of Binned Data]{\includegraphics[keepaspectratio=TRUE,width=.48\textwidth]{BinScatter.pdf}} 
...
\begin{figure}[hbtp]
    \centering
		\subfloat[Spatial Loss]{\includegraphics[keepaspectratio=true, width=.54\textwidth]{./figure/Binning-DataVisualNumericalCenter.pdf}}
...
<<recover,echo=FALSE,results='asis'>>=
set.seed(90)
x <- rnorm(100, 5, 2)
y <- rnorm(100, 5, 2)
...
<<LossTableGraphs,echo=F,results="hide",cache=TRUE>>=
...
\begin{figure}[hbtp]
\centering
<<LossComparison1, echo=FALSE, eval=T, fig.width=6, fig.height=4, out.width='.75\\linewidth', out.height='.495\\linewidth'>>=
...
@
\caption{\label{fig:LossComparison1} Spatial Loss increases with bin width for each variable. The random algorithm has higher spatial loss than the standard algorithm.}
\end{figure}
