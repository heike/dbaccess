...
In the case of modestly sized data, scatterplots are great tools for showing relationships in two dimensions. For large data, scatterplots suffer from over-plotting, %-- i.e.\ a number of observations drawn in such close proximity that their points overlap, 
thereby masking relevant structure.
...
In general, for the univariate case with observations, $x_i$ for $i \in \{1,\dots,n\}$, binning algorithms require a set of bin centers $x_j^\ast$ for $j \in \{1,\dots,J\}$ and a binning function $b_X(.) : x_i \rightarrow x^\ast_j$ that maps observations to  bin centers. 
...
\ktm{Section to describe how data is stored post binning. Section will discuss difference between binned data (bin centers added to original data, still n rows) and "reduced binned data" (aggregated into frequencies), sufficiency for constructing binned scatterplot and storage size.  Want to reference these ideas in later sections.}

% As shown before, the overall loss partitions naturally into visual and numerical loss (see equation \hh{XXX need new equation reference} %\ref{eqn:partition}
% ). We can make use of this and
...
\end{tabularx}}
\end{minipage}
\label{DataReductionExample}
\caption{Original, Binned and Reduced Binned Data Tables, with data storage sizes.  Binned using standard rectangular approach with origin at (-10,-10) and binwidths $\omega_x = \omega_y = 10$. }
\end{table}
...
\begin{itemize}
\item {\it Spatial Loss}, $L^\text{S}$, occurs when points ($x_i$,$y_i$) \hh{XXX define $i$, $l$} in the data set are reduced to a single points at the bin centers ($x_\ell^\ast$,$y_\ell^\ast$). By displaying frequency information using tiles that are visually centered at the bin centers causes a loss of information about the exact location of the points.
...
%
Figure~\ref{fig:spatlossdemo2} visually displays the spatial loss for observations binned using a standard rectangular binning algorithm. Observations $(x_i,y_i)$ and bin centers $\left(x_\ell^\ast,y_\ell^\ast\right)$ are displayed as black points and gray crosses, respectively. The length of line segments connecting these represent $L^S_i$, the spatial loss for each observation; thus, the combined length of all segments represents the total spatial loss, $L^S$.  

For random assignment the total spatial loss can be calculated by simply replacing $b_X(x_i)$ with $b^r_X(x_i)$ in Equation~\ref{totspatloss}.  This total spatial loss for randomly binned data is visualized in Figure~\ref{fig:spatlossdemo2}. Special consideration should be payed to the two points that with spatial losses highlighted in red; in standard binning these points are in neighboring bins and have been allocated to the opposite bin during random binning. We see that the binned scatterplot remains identical if these points are reallocated back to the closer centers; however, the total spatial loss is smaller after the random allocation is post-processed.  

The minimum total spatial loss from all binning combinations that result in the same reduced data, and thus the same binned scatterplot, will be referred to as the \textit{net spatial loss}.  The net spatial loss for standard binned data will always be equivalent to the total spatial loss due to the deterministic bin allocations.  For random binning algorithms the net spatial loss is achieved by exchanging bin assignments for pairs of points in neighboring bins that are exist further from their own bin center that their partner's bin center.
...
Bin counts can be displayed using a continuous shading scale in HSV color space (reference needed) and thus theoretically have no frequency loss. While the tiles of the binned scatterplot would be rendered precisely, the ability of a human with average vision to extract that information using a plot index for the frequency key for the plot is largely imprecise.  Color perception of shade is extremely context sensitive (reference here) allowing an inaccurate mapping of tile shade to the corresponding shade on the frequency key.  
 ...
 Research suggests that even under optimal conditions, we can effectively compare only about seven colors \citep{colorperception}, which provides a physical upper limit on the amount of frequency variation we can perceive.
...
x <- c(rnorm(210, mean=0, sd=0.075), rnorm(210, mean=1, sd=0.075))
y <- c(rnorm(210, mean=0, sd=0.075), rnorm(210, mean=1, sd=0.075))

require(ggplot2)
qplot(x, y, colour=col, size=I(5)) + theme_bw() + theme(aspect.ratio=1)
@
\caption{\label{fig:context}Assess the colors in this scatterplot. Are the darkest spots in the cloud of lighter points as dark as the lightest spots in the cloud of darker points? Darker? Lighter? }
\end{figure}
...
\ktm{Can't figure out how to get rid of the ``washed-out'' look in the continuous color scale figure.  It looks lighter because it is being rendered with tiny gaps between the 1000 narrow tiles which lightens the whole plot because the background is showing through.}

<<FreqLossDemo,echo=F,include=T,eval=T,fig.width=6, fig.height=3, out.width='.8\\linewidth', fig.pos='H',fig.align='center',tidy=F, cache=TRUE, fig.show='hold'>>= 
J= 1
J2 = 200
bound = 1000
