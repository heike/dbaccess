...
Consider the simulated uniform data, we have the fine version and the coarse version, which rounded to nearest even integer. Figure~\ref{fig:strippedplots} displays the binned scatterplots under several scenarios. 
