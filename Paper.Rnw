...
\begin{itemize}
\item {\it Spatial Loss}, $L^\text{S}$, occurs when points ($x_i$,$y_i$) \hh{XXX define $i$, $l$} in the data set are reduced to a single points at the bin centers ($x_l^\ast$,$y_l^\ast$). By displaying frequency information using tiles that are visually centered at the bin centers causes a loss of information about the exact location of the points.
...
require(ggplot2)
qplot(x, y, colour=col, size=I(5)) + theme_bw()
@
