...
\ktm{If the shading scheme for rendering the counts in each bin is discretized there is a loss of frequency information. While counts can certainly be modeled as continuous on the HSV color space (reference needed) discrete color breaks may be preferred for a few reasons. %The average person is able to  On the continuous scale, a viewer will likely not be able to exactly match a tile shading to the corresponding count on the color key in the legend of the plot without an aid such as interactive mouseover effects. 
}
