...
While bin widths can be set to any positive real value, the selection should be restricted to an integer multiple of the resolution of the data to avoid undesirable visual attributes in the binned scatterplot. If the bin dimensions are smaller than the resolution of the data, there will be empty rows or columns of bins throughout the binned scatterplot.  These white-space gaps are undesireable because they create artificial discontinuity that interfers with interpretation of bivariate density. If bin dimensions are not integer multiples of the bivariate data resolution there is also a distruptive visual consequence of ``stripping'', where bins have systematically different numbers of possible data values that can cause artifical density stripes to appear in the binned scatterplot.  

To demonstrate the importance of properly selecting binwidths consider the coarse version of the simulated uniform data which is recorded to a resolution of 2 units. Figure~\ref{fig:UniformStripes} displays the binned scatterplots under several scenarios. Under standard binning we see the white-space gaps with 1 X 1 unit bins, stripes with 5 X 5 unit bins and the correct even spread density with 4X4 unit bins.  Note that here the random binning is effective at eliminating the misleading visual effects when non-integer multiples of the data resolution are selected.
