...
\label{Intro}
% \hh{General: write out any positive integer less than or equal to ten out in words, even if there is the word `unit' behind it. So it's one unit, and not 1 unit.}
% 
% \hh{
% \begin{itemize}
% \item Additional literature: binning papers: standard binning, e.g.: Karl Pearson (1895) ``Contributions to the Mathematical Theory of Evolution. II. Skew Variation in Homogeneous Material"\\
% David W. Scott (1979). "On optimal and data-based histograms". Biometrika 66 (3): 605–610
% 
% quantile binning: there's a lot of implementations (e.g. cut2 in package Hmisc, the rattle package, the package extracut. It would be interesting to check, if they do the same things (probably not) and mention that there is disagreement in the implementations).
% 
% As a reference I would check in 
% Scott, D. W. 1992.
% Multivariate Density Estimation: Theory, Practice, and Visualization. New York: Wiley.
% 
% \item make sure to verify that the binning algorithms agree with the literature (and check that the code is doing the same thing as described in the paper).
% You can include a bit of discussion on the problem that histograms are over-parameterized - it's a known problem, but I can't find any paper on it at the moment.
% 
% \end{itemize}
% }
% 
% \ktm{I have read through the sections of Pearson (1895), Scott (1979) and Scott (1992) that pertained to binning. I will work on integrating them into the lit review next.  Scott talked much about the optimal choice of bin width based on MSE but there was not much talk about what to do with points at the boundaries of bins (he effectively assumed infinite precision in the measurement process to avoid it). 
% 
% I will need to search more to find papers about the binning algorithms and the boundaries they set.  The \texttt{cut} function allows for intervals that are closed on either the left or right and also allows the points on the outermost open bound to be included in the last bin.  
% 
% The \texttt{cut2} function allows the same type of quantile binning as discussed in Scott (1992).}
...
Another approach is to reduce the graphical complexity by plotting binned aggregations of the data, namely frequencies, as opposed to plotting every observation as an individual point. This has the additional advantage of reducing the size of the stored data necessary for the construction of the plot, as only the bin centers and the frequency of observations which correspond to that bin must be stored. Wickham argues for a ``bin-summarize-smooth'' procedure to be applied to the visualization of big data and he notes that simple summary functions, such as counts, scale  well with the size of data \citep{Wickham2013Bin}. Liu, Jiang and Heer employ the computational benefits of binning for their interactive big data visualization program, \textit{imMens} \citep{Liu2013imMens}.  \hh{also add \citep{gold} for binning reference - it's called `weighting' there.}
...
\ktm{Binning algorithms used in making distributional approximations can be traced back to Pearson's work with the binomial approximation to the normal, where he mentions the need to define an origin and binwidth for segmenting the normal distribution \citep{Pearson1895}. More recently Scott has presented discussion on the importance of binning specification in the creation of histograms to appropriately display one dimensional density approximations (\citealp{scott1979} and \citealp{scott1992}). }
...
\end{eqnarray}
%
where $b_X(x_i)$ and $b_Y(y_i)$ are the univariate standard binning algorithms for the X and Y dimensions respectively. The random rectangular binning function, $b^r(\cdot): (x_i,y_i) \rightarrow (x^\ast_\ell,y^\ast_\ell)$ can similarly be defined as
%
...
Data reduction comes at the expense of spatial information of any of the individual points. After aggregation the original spatial locations cannot be recovered. The loss of information incurred from binning will be explored in following sections. 
...
\end{tabularx}}
\end{minipage}
\caption{\label{DataReductionExample}Original, Binned and Reduced Binned Data Tables, with data storage sizes.  Binned using standard rectangular approach with origin at (-10,-10) and binwidths $\omega_x = \omega_y = 10$. }
\end{table}
...
\ktm{
\begin{itemize}
\item {\it Spatial Loss}, $L^\text{S}$, occurs when points $(x_i, y_i)$ for observations $i \in \{1,\dots,n\}$ in the data set are reduced to a set of tiles centered at $(x_\ell^\ast, y_\ell^\ast)$ for bins $\ell \in \{1,\dots,\mathscr{L}\}$. By displaying frequency information using tiles that are visually centered at the bin centers causes a loss of information about the exact location of the points.

\item {\it Frequency Loss}, $L^\text{F}$, occurs when bin counts, $c_\ell \in \{1,\dots,\mathscr{L}\}$ are not rendered precisely. A discrete color scale may be used to aid the ability to read a binned scatterplot, however this is effectively a second stage of binning where shade displays binned frequencies, $b_C(c_\ell) \in \{1,\dots,\mathscr{L}\}$, thus losing precision in the display of the true frequencies. This is analogous to the fundimental problem for overplotted standard scatterplots where frequency is masked by other points. 
\end{itemize}
}

Note that even though these losses may turn out to be substantial, it is, in fact, a huge gain with respect to the original scatterplot, where information is masked in large data situations due to over-plotting of points. \ktm{The idea of loss from one-dimensional binning was explored by Scott using mean squared error as a loss function where the number of bins could be optimized for the construction of histograms \citep{scott1979}. He later extended this discussion to two-dimensional binning, where he compared the mean squared error loss for hexagonal, rectangular and triangular binning; finding that hexagonal and rectangular binning performed similarly, both far superior to triangular binning \citep{scott1992}. We will take a similar approach to quantifying loss resulting from the binning algorithms detailed in Section\ref{GenBinning} above. }
...
L^{\log F} = \sum_{\ell \in \mathscr{L}^\ast} L_\ell^{\log F} = \sum_{\ell \in \mathscr{L}^\ast} (\log(c_\ell) - \log(b_c(c_\ell)))^2
\end{eqnarray}
%
...
All rectangular binning algorithms require the specification of a bin width for each dimension and a binning origin. While using smaller bin widths leads to smaller spatial losses, reducing bin sizes comes at the cost of computation time; a potentially non-negligible consideration in settings with truly massive data. 
 Figure~\ref{fig:SpatialLossCompare} also shows that the computation time needed to bin the simulated data sets is a decreasing function of the bin size and that random binning is marginally slower than standard binning across all bin sizes. Computation times for binning were collected using a commercial Asus laptop with an Intel Core i7 processor running at 2.80 GHz. 
 \ktm{Another pertinent argument for using larger bins is that it will smooth over  larger regions if visualizing the large scale density structure is of primary interest.}
\hh{This is an argument along the lines of the bias vs variability tradeoff. Extend on it, don't put it here as a throwaway sentence. }

While bin widths can be set to any positive real value, the selection should be restricted to an integer multiple of the resolution of the data to avoid undesirable visual attributes in the binned scatterplot. If the bin dimensions are smaller than the resolution of the data, there will be empty rows or columns of bins throughout the binned scatterplot.  These white-space gaps are undesirable because they create artificial discontinuity that interferes with interpretation of bivariate density. If bin dimensions are not integer multiples of the bivariate data resolution there is also a disruptive visual consequence of ``stripping'', where bins have systematically different numbers of possible data values that can cause artificial density stripes to appear in the binned scatterplot.  

To demonstrate the importance of properly selecting binwidths consider the coarse version of the simulated uniform data which is recorded to a resolution of 2 units. Figure~\ref{fig:UniformStripes} displays the binned scatterplots under several scenarios. Under standard binning we see the white-space gaps with 1 X 1 unit bins, vertical and horizontal stripes with 5 X 5 unit bins and the appropriate view of an evenly spread density with 4X4 unit bins.  Note that random binning is effective at smoothing out the stripped patterns when non-integer multiples of the data resolution are selected.

<<UniformStripes, echo=F,include=T,eval=T,fig.width=8, fig.height=5, out.width='.9\\linewidth', fig.pos='h',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="Binned scatterplots of coarse uniform data with square bins of size 1, 4 and 5">>= 
...
For data with a coarse resolution the location of the origin is also important because the origin controls the proximity of possible data values to bin centers. It can be shown analytically that for data with a resolution of $\alpha$ units with a  symmetric distribution within each of the bins, the origin that minimizes net spatial loss is located $\alpha/2$ units below the minimum value (see proof in Appendix~\ref{proof:offset}).

\hh{XXX which bin size are you talking about? add this, otherwise the statement is wrong XXX}

\hh{`$\alpha/2$ origin offset' this an implicit naming. You have to make it either explicit or write this out. At the moment it is imprecise, because you mean this offset to be below the minimal value of the data. 
XXX }

In practice the $\alpha/2$ origin offset is found to be a reasonable choice for minimizing spatial loss for symmetric bivariate data at resolution $\alpha$ while using bin dimensions that are integer multiples of $\alpha$.
...
Figure~\ref{fig:OriginOffsetSimulations} shows how the net spatial loss changes as the origin is slide from the minimum value to one binwidth below the minimum value using the coarse simulated data with a resolution of 2 units. \ktm{The round glyphs indicade the origin offset where the net loss reached an absolute minimum. For the two symmetrically distributed data sets, normal and uniform, the origin offset of 1 unit was found to either minimize the net spatial loss or achieve a local minimum very near to the overall minimum (within 0.5\% of the magnitude of the minimum spatial loss) for each considered bin size.} %\hh{XXX - no! the value at -1 is close, but the dots are in the actual minimal values. This is only a single example - the expected minimum might still be in -1. }
For the bivariately skewed exponential data, the 1 unit origin offset minimized net loss for the smaller intervals but was not optimal for the largest intervals.
...
\ktm{ Figure~\ref{freqbinning} shows binned scatterplots resulting from standard and quantile frequency binning algorithms using five frequency bins. Note that for standard binning, the shade is to be interpreted as an ordinal indicator of the groups based on bin frequencies, whereas for quantile binning the shade denotes groups based on frequency quantiles.  This distinction explains why the shading of the binned scatterplot is invariant to the log transformation for quantile frequency binning but not for standard; because a monotone transformation will not alter groupings based on order, but will change groupings based on value magnitude.}

The top row in Figure~\ref{fig:FreqLossCompare} displays the frequency loss from using standard and quantile frequency binning algorithms, with between one and ten frequency bins, from each set of simulated data. We first notice that large difference in the magnitude of frequency losses based on the bivariate distribution; frequency losses are highest for the exponential data, lower for the normal data and lowest for the uniform data. We also note that the frequency loss is a decreasing function of the number of frequency bins for both binning algorithms. This relationship largely flattens out after four frequency bin, each subsequent bin reducing the frequency loss less than the previous. Thus we should consider using between four and seven bin shades in order to reduce loss while also allowing for easy perception of frequency groups in the binned scatterplot. Lastly, the frequency loss with the standard frequency binning is be higher than for the quantile-based algorithm when a minimal number of bins are used. But in all three simulated data sets, the losses drop to comparable levels for each algorithm when four or more frequency bins are used.
...
<<noFreqBin, echo=FALSE, out.width='\\linewidth',fig.width=4, fig.height=4>>=
...
<<FreqBinPlots, echo=FALSE, out.width='\\linewidth',fig.width=10, fig.height=10>>=
freqgrps = 5
...
@
\end{minipage}
\caption{\label{freqbinning}Simulated bivariate normal data spatially binned using standard algorithm with 5X5 unit bins and origin at (0,0). Binned Scatterplot with raw frequencies (left) and binned scatterplots with five frequency bins (right). Bin shades labeled by frequency range and average. }
\end{figure}
...
The exploration of loss of spatial and frequency information in the process of creating a binned scatterplot yielded a number of important properties and practical recommendations for their construction. Spatially binning with smaller bin dimensions will lead to lower spatial losses; however, finer binning requires more processing time and does not highlight large scale density structure. Ideally we would like to strike the optimal trade-off between bin size and spatial loss, but unfortunately the simulated data from the three bivariate distributions indicated that this relationship between them was nearly linear. Thus it is left to the plot designer to decide if they wish to highlight high level density structure with higher spatial loss or finer density structure with lower spatial loss.

For spatial binning, the random binning algorithm displayed strong advantage of avoiding the visual ``stripes'' problem that occurred when data recorded to a coarse resolution was binned using a bin width that was a non-integer multiple of the resolution, $\alpha$. However, standard binning algorithm is superior due to lower spatial loss, and data with coarse resolution of $\alpha$ units can avoid the stripe illusion if a binwidth that is an integer multiple of $\alpha$ is selected.  It was also shown that in this scenario, a reasonable default for the binning origin is offset to $\alpha / 2$ units below the minimum data value because it will minimize spatial loss for symmetric data and generally preform well even for non-symmetric data. 

If we elect to use frequency binning to aid the perceptual mapping tile shades to the plot key for frequency, it is recommended to use between four and seven distinct shades.  This is done to minimize the frequency loss within the bounds of human perceptual ability to distinguish multiple shades simultaneously. The standard and quantile algorithms for frequency binning have very similar loss properties for this range, so the decision becomes one of how we wish the shade of a tile to be interpreted. Standard log frequency binning is an alternative if the bin counts are heavily right skewed and it is desired to differentiate bins with low density. We also discovered that if we elect to use quantile binning, the log transformation will have no effect on the binned scatterplot shading due to the identical quantile-based mapping to ordinal frequency groupings for all monotone transformations on bin counts.  

These recommendations for binned scatterplot construction and loss properties will be demonstrated in the following examples using real bivariate data. First we will further explore the baseball pitching data. Additionally we will investigate the relationship between scheduled and actual departure times for a full year of American commercial airline data. 
...
\ktm{We have presented two algorithms for spatially binning data points; the standard algorithm, which has rounding artifacts but generally lower loss, and the random algorithm, which is a fast version of linear binning and is somewhat more computationally efficient. The standard algorithm is superior in terms of net spatial loss, but requires more care in selecting binning parameters than random binning to avoid generating problematic visual features. In setting binning parameters for each spatial dimension, binwidth should be an integer multiple of the data resolution and the origin should be offset half the data resolution below the lowest observed value. }

\ktm{We also have explored the loss of frequency information for cases where frequencies are also binned to create a discrete color scale for tile shading. It is recommended to use less than ten seperate shades due to bounds of perceptual discrimination but at least four shades to avoid the steep frequency losses seen with fewer frequency bins. Using quantile frequency binning and log transforming the bin counts prior to standard frequency binning are shown to be reasonable methods -- with slight different interpretability -- for handling situations with heavily skewed bin count distributions.  }
