...
d1.ext <- xtd.data(d1)
@
<<scatterplots, dependson='setup', echo=F, fig.keep='none', results='hide'>>=
...
<<binningDemo,echo=F, fig.keep='none', cache=T>>=
      library(mvtnorm)
      set.seed(33)
...
<<RandomBinDemo,echo=F,include=FALSE,fig.keep='none', cache=TRUE>>=
nl <- ncol(d.rand)
dnew <- d.rand
binning <- c(10, 10)
for(x in 1:(nl)){
...
\begin{figure}[hbtp]
    \centering
		\subfloat[Spatial Loss]{\includegraphics[keepaspectratio=true, width=.54\textwidth]{./Binning-DataVisualNumericalCenter.pdf}}
