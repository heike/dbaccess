...
%
\ktm{ \begin{eqnarray}\label{randbin1}
...
%   {./figure/RandomBinningDataPoints1}}
  ...
  \caption{Visualization of how data pairs are assigned to bins centered at (5, 25) and (15,25) under the two rectangular binning algorithms. Points in the standard algorithm are contained within the defined bin region, where points from the random algorithm may be assigned to bins from outside the defined bin regions.}
  \label{fig:BinningTutorial}
\end{figure}


\subsection{Quantile Binning Algorithm}
