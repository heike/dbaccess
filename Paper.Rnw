...
If we elect to use frequency binning to aid the perceptual mapping tile shades to the plot key for frequency, it is recommended to use between four and seven distinct shades.  This is done to minimize the frequency loss within the bounds of human perceptual ability to distinguish multiple shades simultaneously. The standard and quantile algorithms for frequency binning have very similar loss properties for this range, so the decision becomes one of how we wish the shade of a tile to be interpreted. Standard log frequency binning is an alternative if the bin counts are heavily right skewed and we wish to find distributional structure in bins with low density. We also discovered that if we elect to use quantile binning, the log transformation will have no effect on the binned scatterplot shading due to the identical quantile-based mapping to ordinal frequency groupings for all monotone transformations on bin counts.  
