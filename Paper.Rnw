...
\author{Susan VanderPlas$^1$, Heike Hofmann$^{1,2}$\\$^1$Department of Statistics, $^2$Human Computer Interaction\\Iowa State University}
...
# setwd("/home/susan/Dropbox/GraphicsGroup/")
setwd("/Users/heike/Dropbox/papers/")

setwd("./dbaccess/NewWriteUp/")
...
qplot(data=d1, x=G, y=SO, geom="point", main="Games and Strike Outs in ML Baseball", shape="o", size=I(6)) + scale_shape_identity() + theme_bw()
  ggsave("./images/OverplottingCircles.pdf", width=5, height=5)
qplot(data=d1.ext, x=G, y=SO, geom="hex", fill=..count.., group=1) +  
  ...
  ggsave("./images/HexBinning.pdf", width=5, height=5)
...
qplot(data=d1.hex, x=G, y=SO, geom="point", size=Freq) + 
  ...
  ggsave("./images/BinScatter.pdf", width=5, height=5)
...
\begin{figure}[hbtp]
  \subfloat[Overplotted data ]{\includegraphics[keepaspectratio=TRUE,width=.31\textwidth]{images/Overplotting.eps}}
  \subfloat[Tukey-style open circles]{\includegraphics[keepaspectratio=TRUE,width=.31\textwidth]{images/OverplottingCircles.eps}} 
	\subfloat[Alpha blending]{\includegraphics[keepaspectratio=TRUE,width=.31\textwidth]{images/OverplottingAlpha.eps}}
  
  \subfloat[Hexagonal binning]{\includegraphics[keepaspectratio=TRUE,width=.48\textwidth]{images/HexBinning.eps}}
  \subfloat[Scatterplot of Binned Data]{\includegraphics[keepaspectratio=TRUE,width=.48\textwidth]{images/BinScatter.eps}} 
...
As alternatives, Figure~\ref{fig:scatter-alpha} (d-e) contains examples of a hexagonally binned scatterplot with frequency encoded as color (d) and a ``bubble plot" (referred to as a modified scatterplot) with frequency encoded as point size (e). The hexagonal tiles and the modified scatterplot are more effective at displaying the shape of the joint density and preserving outliers than any of the scatterplots shown in Figure~\ref{fig:scatter-alpha} (a-c). The binned scatterplot is less prone to the Hermann-grid illusion than the bubble plot, particularly for binned data, where bin centers usually fall on a grid.
...
<<binningDemo,echo=F,eval=F,include=F,cache=T>>=
      library(mvtnorm)
      set.seed(33)
      ...
      ggsave("./images/Binning-DataAndBin.pdf", width=5, height=5, units="in")
...
#                  label=expression(paste('(', over(phantom(0),x)[i], ',', over(phantom(0),y)[i],')'))) +
      ...
      ggsave("./images/Binning-DataVisualNumericalCenter.pdf", width=5.5, height=3.25, units="in")
@
...
\begin{figure}[hbtp]
  \centering
  \subfloat[Standard Algorithm]{\includegraphics[keepaspectratio=true, width=3in]{./images/Binning-DataAndBin.eps}}
%   \subfloat{\includegraphics[keepaspectratio=TRUE, width=2in]
%   {./images/RandomBinningDataPoints1}}
  \subfloat[Random Algorithm]{\includegraphics[keepaspectratio=TRUE, width=3in]
  {./images/RandomBinningDataPoints2.eps}}
  \caption{Points are assigned to bins centered at (5, 25) an (15,25). Points in the standard algorithm are all contained within the bin region, where points in the random algorithm may be outside the direct region.}
  \label{fig:BinningTutorial}
\end{figure}

\subsection{Random Binning Algorithm}
<<RandomBinDemo,echo=F,include=FALSE,eval=F>>=
nl <- ncol(d.rand)
dnew <- d.rand
binning <- c(10, 10)
for(x in 1:(nl)){
...
ggsave("./images/RandomBinningDataPoints2.pdf",width=5, height=5)
@
...
\item {\it Visual Loss} is an implicit loss due to the rendering;  visually we perceive the center of a bin to be the center of the tile. This visual center will generally different from the numerical center, or the mean of the data corresponding to that bin, which can be thought of as the center of mass. Thus visual loss is the difference between the visual center $(x^\ast, y^\ast)$ and the numerical center $(\bar{x}, \bar{y})$. This causes some bias in the perception.

\item {\it Numerical Loss} occurs when points in the data set are reduced to a single point at the mean, i.e. this is the collective difference between all data points $(x_i,y_i)$ and their corresponding numerical centers $(\bar{x}, \bar{y})$.
...
Numerical loss results from the binning process as points are reduced to the numerical center point and is thus the sum of numerical losses for individual points ($\sum_{i=1}^n L_i^\text{N}$).
Visual loss is due to the difference between the numerical centers and the visual centers and is thus the sum of visual losses for each bin ( $\sum_{j=1}^{N_x} L_j^\text{V}$). This discrepancy is particularly clear as demonstrated in Figure~\ref{fig:stdBinTutorial2}, in which the visual center of the bin varies significantly from the numerical center of the data contained within the bin under the standard algorithm.
...
ggsave("./images/FreqLoss.pdf", width=5.4*.8, height=4.05*.8)
@
...
The loss as a function of bin width was computed for both algorithms for the baseball data and is shown in Figure~\ref{fig:LossComparison1}.

\begin{figure}[hbtp]
    \centering
		\subfloat[Spatial Loss]{\includegraphics[keepaspectratio=true, width=.54\textwidth]{./Binning-DataVisualNumericalCenter.eps}}
    \subfloat[Frequency Loss]{\includegraphics[keepaspectratio=true, width=.45\textwidth]{./images/FreqLoss.eps}}
...
\begin{figure}[hbtp]
\centering
<<FrequencyCategories,echo=FALSE, results='hide'>>=
...
<<LossTableGraphs,echo=F,results="hide",cache=TRUE,eval=FALSE>>=
LossTable <- NULL
...
computeLoss <- function(binning, data){
  ...
  datplot <- rbind(cbind(lRandom$NewData, Algorithm="Random"), cbind(lMean$NewData, Algorithm="Standard"))
    ...
    ggsave(paste("./images/RandomStd", binning[1], binning[2], ".pdf", sep=""), width=8, height=5, units="in", dpi=1200)
...
#     ggsave(paste("./images/Standard", binning[1], binning[2], ".pdf", sep=""), width=4.5, height=4.5, units="in")
  return(rbind(
...
%Figure~\ref{fig:binning-comparison} shows clearly that the standard algorithm can produce misleading graphical summaries. 
 ...
 These characteristics of the numerical and visual losses are shown using the baseball data in the total combined visual and numerical loss at different bin widths in $x$ (Games) in Figure ~\ref{fig:LossComparison1}.
...
qplot(x=x, y=TotalLoss.G*100, group=interaction(y, Algorithm), data=subset(losses, y<=10), geom="line", colour=Algorithm, ylab="Percent Spatial Loss", xlab="Bin  Width  (Games)")+ theme_bw() + theme(legend.position="bottom") + ggtitle("Total Combined Numerical and Visual Loss:  Baseball Games") + guides(colour = guide_legend(override.aes = list(size = 2))) + facet_wrap(~Algorithm)
...
For this data example we will revisit the baseball data used earlier in this paper.
...
We may also use this example to highlight the advantages of random binning. The two binned scatterplots at the top of Figure~\ref{fig:binning-comparison} show bins of the same width, but clear differences between the binning results are evident: while the standard binning algorithm has rounding artifacts that are clearly visible for even bin widths, the random algorithm produces a graph that better represents the true shape of the data. This is because the In contrast, when the bin widths are odd, the two binning algorithms yield nearly indistinguishable binned scatterplots, as shown in the bottom row of Figure~\ref{fig:binning-comparison}.

\begin{figure}[hbtp]
\centering
\subfloat[(2, 2) tiling of G, SO data]{
\includegraphics[keepaspectratio=true, width=.9\textwidth]{images/RandomStd22.eps}
}\\
\subfloat[(5, 25) tiling of G, SO data]{
\includegraphics[keepaspectratio=true, width=.9\textwidth]{images/RandomStd525.eps}
}
...
\subfloat[Million-point sample of Airline Data]{\includegraphics[width=.44\linewidth, keepaspectratio=TRUE]{./data/MillionSample.eps}}\hfil
\subfloat[Minimal binning of Airline Data]{\includegraphics[width=.56\linewidth, keepaspectratio=TRUE]{./data/AirlineStdBinning1min.eps}}
...
\begin{figure}[hbtp]\centering
\includegraphics[keepaspectratio=true, height=.43\textheight]{./data/AirlineStdBinning5mins.eps}
\includegraphics[keepaspectratio=true, height=.43\textheight]{./data/AirlineStdBinning15mins.eps}
