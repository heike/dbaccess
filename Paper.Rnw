...
<<SpatialLossCompare,echo=F,include=T,eval=T,fig.width=8, fig.height=3, out.width='.8\\linewidth', fig.pos='h',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="Lineplots for net spatial loss over a range of bin sizes for standard and random binning of the fine version of the simulated data from each bivariate distribution.">>= 
...
All rectangular binning algorithms require the specification of a bin width for each dimension and a binning origin. While using smaller bin widths leads to smaller spatial losses, reducing bin sizes comes at the cost of computation time; a non-negligible consideration in settings with truly massive data.
...
For data with a coarse resolution the location of the origin is also important because the origin controls the proximity of possible data values to bin centers. It can be shown analytically that for data with a resolution of $\alpha$ units that is spread symmetrically within bins, the origin that minimizes net spatial loss is located $\alpha/2$ units below the minimum value (see proof in Appendix~\ref{proof:offset}).

In practice the $\alpha/2$ origin offset is found to be a reasonable choice for minimizing spatial loss for symmetric bivariate data at resolution $\alpha$ while using bin dimensions that are integer multiples of $\alpha$. Figure~\ref{fig:OriginOffsetSimulations} shows how the net spatial loss changes as the origin is slide from the minimum value to half the binwidth below the minimum value using the coarse simulated data with a resolution of 2 units. For the two symmetrically distributed data sets, normal and uniform, the origin offset of 1 unit below the minimum data value was found to minimize the net spatial loss for each considered bin size (2, 4, 6, 8, and 10 units). For the bivariately skewed exponential data, the 1 unit origin offset minimized net loss for the smaller intervals but was not optimal for the larger intervals.

<<OriginOffsetSimulations, echo=F,include=T,eval=T,fig.width=8, fig.height=5, out.width='.9\\linewidth', fig.pos='h',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="Net spatial loss for coarse simulated data at a resolution of 2 units at various sized (2,4,6,8,10) square bins over the range of possible origin offsets. The vertical gray line in each facet indicates where the origin is offset 1 unit below the minimum data value.">>= 
...
#   for (width in widths){
#     for (origin in seq(-width/2,0, by=width/60)){
#       index <- index+1
...
\ktm{
The reduced binned data from spatial binning contains the center and count information for all bins.  The bin frequencies may themselves be binned in order to discretize the color scale for the binned scatterplot. The loss of frequency information in frequency binning is dependent on the selection of a discrete color mapping, where the binning algorithm and number of frequency bins must be specified. To explore frequency loss characteristics using the simulated bivariate data sets, each of the three fine resolution sets were first spatially binned using standard rectangular binning to produce three sets of reduced binned data; then standard, quantile and log frequency binning algorithms were applied with various numbers of frequency bins. As discussed in Section~\ref{FreqLoss}, we will limit the considered options to using between one and seven frequency bins due to the upper limits of human perceptual abilities \citep{colorperception}.  

Figure~\ref{fig:FreqLossCompare} displays the frequency loss from using standard and quantile frequency binning algorithms, with between one and seven frequency bins, from each set of simulated data. We first notice that large difference in the magnitude of frequency losses based on the bivariate distribution; frequency losses are highest for the exponential data, lower for the normal data and lowest for the uniform data. We also note that the frequency loss is a decreasing function of the number of frequency bins for both binning algorithms, thus we should consider using the maximum number of bin shades that human perception is able to simultaneously discern.  Lastly, the frequency loss with the standard frequency binning may be higher than for the quantile-based algorithm when a minimal number of bins are used; but in all three simulated data sets, this is reversed when seven frequency bins are used. Thus we recommend using the standard algorithm over the quantile-based algorithm for frequency binning because it had smaller frequency loss at the recommended seven bins, and additionally carries the benefit that equally dividing the bin count range into equally spaced bins creates a simpler shading interpretation than dividing by quantiles.

Due to the difference in scales between counts and log counts, the frequency loss and log frequency loss can not be directly compared. Frequency binning using the log counts did however behave very similarly, where log frequency loss decreased as the number of frequency bins increased.  Thus the same advise to use seven frequency bins holds for shading based on log counts as well. 
}

<<FreqLossDisplay,echo=F,include=T,eval=T,fig.width=15, fig.height=5, out.width='.99\\linewidth', fig.pos='H',fig.align='center',tidy=F, cache=TRUE, warning=F, fig.show='hold',fig.cap="I am not sure that this figure is needed to demonstrate frequency binning. If so, do the colors between freq bins need to be made more distinguishable? i.e. in the quantile binning the bins starting at 1, 8 and 36.5 are hard to differentiate because the shade is determined by the breaks. ">>= 
...
p1 <- qplot(binxs, binys, geom="tile", fill=binfreq, data=binoutFreqGroups[[1]],
...
p2 <- qplot(binxs, binys, geom="tile", fill=freqgroup, data=binoutFreqGroups[[1]],
...
p3 <- qplot(binxs, binys, geom="tile", fill=freqgroup, data=binoutQuantFreqGroups[[1]],
...
p4 <- qplot(binxs, binys, geom="tile", fill=freqgroup, data=binoutLogFreqGroups[[1]],
            main="Log Freq Binning") +
    xlab("") + ylab("")+
...
<<FreqLossCompare,echo=F,include=T,eval=T,fig.width=10, fig.height=4, out.width='.99\\linewidth', fig.pos='H',fig.align='center',tidy=F, cache=TRUE, warning=F, fig.show='hold',fig.cap="Lineplots for total frequency loss from standard and quantile frequency binning of reduced binned data from each bivariate distribution. Spatial binning to produce the reduced binned data done with standard rectangular binning with 5 X 5 unit bins and origin at (0,0).">>= 
...
p1 <- qplot(ncolor, totalFreqLoss, geom="path",linetype=FreqBinType,
  ...
  scale_x_continuous(breaks=1:7) + ylim(0,max(plotdat$totalFreqLoss)) +
...
<<FreqLossCompare2,echo=F,include=T,eval=T,fig.width=10, fig.height=4, out.width='.99\\linewidth', fig.pos='H',fig.align='center',tidy=F, cache=TRUE, warning=F, fig.show='hold',fig.cap="Lineplots for total log frequency loss from log frequency binning of reduced binned data from each bivariate distribution. I don't think this plot will make the final cut, but it is interesting that after logging the bin counts, the log freq loss for the uniform data does not improve with more bins (might as well use one shade for all bins)">>= 
### Loop over ncolors 1 to 7 and also over data distribution
# find freq loss from standard, quantile, logCount
freqLossSimDat <- NULL
distrib <- c("Uniform", "Normal", "Exponential")
...
plotdat <- freqLossSimDat[(freqLossSimDat$FreqBinType=="Log Count" & freqLossSimDat$DistType=="Exponential"),]
p1 <- qplot(ncolor, totalFreqLoss, geom="path",linetype=FreqBinType,
      data=plotdat , color = I(3), size=I(1)) +
  facet_grid(.~DistType, scales="free_y") +  theme_bw() +
  scale_x_continuous(breaks=1:7) + ylim(0,max(plotdat$totalFreqLoss)) +
...
\ktm{
The exploration of loss of spatial and frequency information in the process of creating a binned scatterplot yielded a number of important properties and practical recommendations for their construction. Spatially binning with smaller bin dimensions will lead to lower spatial losses; however, finer binning requires more processing (Would like to use an O(n)-type argument, but I don't rightly understand this comp. sci. notation) and does not highlight large scale density structure. Ideally we would like to strike the optimal trade-off between bin size and spatial loss, but unfortunately the simulated data from the three bivariate distributions indicated that this relationship between them was nearly linear. Thus it is left to the plot designer to decide if they wish to highlight high level density structure with higher spatial loss or finer density structure with lower spatial loss.

For spatial binning, the random binning algorithm displayed strong advantage of avoiding the visual ``stripes'' problem that occurred when data recorded to a coarse resolution was binned using a bin width that was a non-integer multiple of the resolution, $\alpha$. However, standard binning algorithm is superior due to lower spatial loss, and data with coarse resolution is not problematic so long as a binwidth that is an integer multiple of $\alpha$ is selected.  It was also shown that in this scenario, we should use an origin that is offset to $\alpha / 2$ units below the minimum data value leads to minimize spatial loss. 

If we elect to use frequency binning to aid the perceptual mapping tile shades to the plot key for frequency, it is recommended to use seven distinct shades.  This is done to minimize the frequency loss within the bounds of human perceptual ability to distinguish multiple shades simultaneously. Standard frequency binning is recommended over the quantile frequency binning because it displayed lower frequency loss for all three simulated binned data sets while using seven frequency bins.  Log frequency binning is an alternative if the bin counts are heavily right skewed and it is desired to differentiate bins with low density.  
