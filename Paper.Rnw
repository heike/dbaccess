...
\ktm{ As previously mentioned second stage of binning requires that we count the number of observations assigned to each unique bin center; forming data triples, $(x^\ast, y^\ast, c)$, where $c$ is the number of all observations assigned to bin center $(x^\ast,y^\ast)$. Table~\ref{DataReductionExample} uses a small set of simulated data to show the progression from the original data (a), to the binned data (b), to the reduced binned data (c).  The reduced binned data is sufficient for constructing the binned scatterplot, as only bin centers and frequencies are necessary.  As such the storage size for the information and the computation time needed to construct the plot is greatly reduced. }

\ktm{ The data reduction comes at the expense of spatial information of any of the individual points. After the binned data is aggregated, there is no way to recover the original spatial locations. The loss of information incurred from binning will be explored in following sections. }
