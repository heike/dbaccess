...
%Histograms are an example of plots which use binned variables. \hh{XXX No, histograms don't use binned variables in their standard definition, histograms bin variables to render the information. Weighted histograms (again, \citet{gold}) make use of binned data, because we can think of the tabular information as the sufficient statistics for histograms.} 
...
The second stage of binning requires a frequency breakdown of the number of observations associated with each bin center, forming data triples, $(x^\ast, y^\ast, c)$, where $c$ is the number of all observations assigned to bin center $(x^\ast,y^\ast)$. Table~\ref{DataReductionExample} makes use of a small set of simulated data to show the progression from the original data (a), to the binned data (b), to the reduced binned data (c).
The reduced binned data is sufficient for constructing the binned scatterplot. %, as only bin centers and frequencies are necessary.  
Binning greatly reduces the storage size for the information and the computation time needed to construct the plot.
\hh{XXX this is not always true - when the bins have only 1s the binned data set is actually bigger than the original. }

Data reduction comes at the expense of spatial information of any of the individual points.
After aggregation the original spatial locations cannot be recovered. The loss of information incurred from binning will be explored in following sections. 
...
\begin{itemize}
\item {\it Spatial Loss}, $L^\text{S}$, occurs when points $(x_i, y_i)$ for observations $i \in \{1,\dots,n\}$ in the data set are reduced to a single points at the bin centers $(x_\ell^\ast, y_\ell^\ast)$ for bins $\ell \in \{1,\dots,L\}$. By displaying frequency information using tiles that are visually centered at the bin centers causes a loss of information about the exact location of the points.

\item {\it Frequency Loss}, $L^\text{F}$, occurs when frequency information is not rendered, such as in standard scatterplots,  or not rendered accurately, e.g.\ when a discrete color scale is used to display bin counts, the entire ranges of counts is mapped to the same color; thus losing precision in the display of frequencies.
\end{itemize}
...
\ktm{ For random assignment the total spatial loss can be calculated by simply replacing $b_X(x_i)$ with $b^r_X(x_i)$ in Equation~\ref{totspatloss}.  The total spatial loss for randomly binned data is visualized in Figure~\ref{fig:spatlossdemo2}. Special consideration should be payed to the two points that with spatial losses highlighted in red; in standard binning these points are in neighboring bins and have been allocated to the opposite bin during random binning. We see that the binned scatterplot remains identical if these points are reallocated back to the closer centers; however, the total spatial loss is smaller after the random allocation is post-processed.  }

\ktm{ The minimum total spatial loss from all binning allocations that result in the same reduced binned data, and thus the same binned scatterplot, will be referred to as the \textit{net spatial loss}, $L^S_{net}$.  The net spatial loss for standard binned data will always be equivalent to the total spatial loss due to the deterministic bin allocations.  For random binning algorithms the net spatial loss is achieved by first exchanging bin assignments for pairs of points in neighboring bins that exist further from their own bin center than from their partner's bin center, then finding the total spatial loss from this processed binned data. }

\ktm{ The net spatial loss is a euclidean distance, but requires a reference scale on which the magnitude may be interpretable. We will use the spatial loss from a ``worst-case'' binning scenario as a reference value. The largest displacement of points will occur if we place all points into a single bin.  While the total spatial loss for single bin could be made arbitrarily large if the bin center is placed far away from the data values, we will consider a reasonable specification of a bin with boundaries that span only the range of the data in each dimension; thus centered at  $( (x_{(1)}-x_{(n)})/2$ , $(y_{(1)}-y_{(n)})/2 )$.  The total spatial loss for this ``worst-case'' bin will be referred to as the \textit{potential spatial loss}, $L^S_p$, which is defined as }
\begin{eqnarray}\label{potspatloss}
L^S_p = \sum_{i=1}^{n} \sqrt{ \left(x_i-(x_{(1)}-x_{(n)})/2\right)^2 + \left(y_i-(y_{(1)}-y_{(n)})/2\right)^2 }
\end{eqnarray} 
%
\ktm{ Thus the net spatial loss, as a proportion of potential spatial loss, $L^S_{net}/L^S_p$, will be used as the primary metric for assessing spatial loss incurred from binning.} 
...
\hh{It is therefore not realistic, to expect readers to be able to map colors to a continuous color scheme, even though theoretically we certainly can distinguish enough different colors (see e.g.\ the discussion in \url{http://hypertextbook.com/facts/2006/JenniferLeong.shtml}). Figure~\ref{fig:context} shows an example of the context sensitivity of colors. Even though there is a lot of spread between the colors in this example, most onlookers have trouble answering the question about the relative relationship of the shading of the darkest/lightest points in each of the points clouds.
}
...
L^F = \sum_{\ell=1}^{L} L_\ell^F = \sum_{\ell=1}^{L} (c_\ell-b_c(c_\ell))^2
\end{eqnarray}  
%
...
L^{\log F} = \sum_{\ell=1}^{L} L_\ell^{\log F} = \sum_{\ell=1}^{L} (\log(c_\ell+1) - \log(b_c(c_\ell+1)))^2
\end{eqnarray}
%
Note that we have added one count to every cell before taking the logarithm to avoid problems with empty cells. 

\ktm{ As with spatial binning, there will be a ``worst-case'' scenario for frequency binning that will provide context to the magnitude of the total frequency loss.  If we elect to use a single frequency bin then all tiles of the binned scatterplot will be rendered with the same shade; thus the shade would represent the average bin density, $\bar{c} =  (\sum_{\ell=1}^{L} c_\ell) /L$.  We define the total frequency loss in the single shade scenario as the \textit{protential frequency loss}, $L^F_p$,  where
%
\begin{eqnarray}\label{potfreqloss}
L^F_p = \sum_{\ell=1}^{L} (c_\ell-\bar{c})^2.
\end{eqnarray}  
%
Thus total frequency loss after binning will be interpreted through the proportion of potential frequency loss, $L^F / L^F_p$.  Similarly we define the \textit{protential log frequency loss}, $L^{\log F}_p$, as 
%
\begin{eqnarray}\label{potlogfreqloss}
L^{\log F}_p = \sum_{\ell=1}^{L} (\log(c_\ell+1)-\log(\bar{c}+1))^2,
\end{eqnarray}  
% 
which can then be used to find the proportion of potential log frequency loss, $L^{\log F} / L^{\log F}_p$, in cases where log frequency binning was conducted. } %end ktm{} red
...
\paragraph{Traditional Scatterplots and Frequency Information: (old writeup - to be reworked or removed)}
...
\ktm{ Binning data for the purpose of creating a binned scatterplot requires a choice of algorithm and also the parameters of the chosen binning algorithm.  This section aims to compare binning algorithms and identify the best parameter choices for minimizing loss under a number of scenarios.  Some results are derived optimal choices through analytical proofs while others are recommendation based on numerical from empirical exploration of loss from binning.  } 

\ktm{ Whether analytical or empirical, data is needed to demonstrate how loss is impacted by binning choices. Bivariate datasets were simulated from bivariate distributions to be used throughout this section: Exponential, Normal and Uniform.  These distributions were selected for their variety in terms of shape, center and spread. 100,000 observation pairs were simulated for each set of simulated data, and the x and y values were drawn independently from univariate distributions; Exponental(11), Normal(50,$11^2$) and Uniform(0,100), respectively.  The parameters were selected to have data values roughly span the region $[0,100]^2$. The simulated data can be found in the top row of Figure~\ref{fig:SimDataScatterplots}. }

<<SimDataScatterplots,echo=F,include=T,eval=T,fig.width=8, fig.height=6, out.width='.8\\linewidth', fig.pos='h',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="Scatterplots of fine and coarse versions of the simulated bivariate data.">>= 
...
\ktm{ These simulated datasets are from continous distributions, and thus the values are recorded to many decimal places. Real data is recorded to only the number of digits that the measurements allow; in many cases rounded to the nearest whole dollar or five minutes or two miles. To observe binning loss under more realistic conditions, three corresponding data sets were also created by rounding the values from the original simulated data sets to the nearest even number. This coarse version of the original simulated data is displayed in the bottom row of Figure~\ref{fig:SimDataScatterplots}. By exploring the loss properties for both the \textit{fine} and \textit{coarse} versions of the data, we identify which binning options are robust to the resolution to which data is recorded. }

%-----------------------------------------------------------------------------

\subsection{Rectangular Binning Specifications and Spatial Loss}

For rectangular binning there are many specification options; including type of algorithm, location of the origin and binwidths for each dimension. To explore the spatial loss properties under different binning approaches we begin with a comparison of standard and random binning. If equivalent bin widths are used, the net spatial loss under standard binning will always be less than or equal to the net spatial loss under random binning. This is because the minimal spatial loss for each data point under random binning is to allocate to the nearest bin center, which is how the point would be allocated in standard binning. 

Figure~\ref{fig:SpatialLossCompare} displays the proportion of potential spatial loss from binning each set of simulated data with standard and random binning algorithms using square bins with a sizes ranging from 2 units$^2$ to 20 units$^2$. It is clear to see that the spatial loss for random binning is always higher than for standard binning and that the loss grows as binwidth increases. The proportion of potential spatial loss from random binning increases with binwidth at a faster rate than with standard binning, as indicated by the widening gap and steeper slopes.  

Another attribute that stands out in Figure~\ref{fig:SpatialLossCompare} is that proportion of potential spatial loss is much lower for the exponential data than for the normal or uniform data; this is because the potential spatial loss is much higher for skewed data than symmetric. This is not surprising when we consider that if a single rectangular tile were placed over the bivariate range of the exponential data, the center of the tile would be a fair distance away from the center of mass for the points. The plot is reflecting that the ``worst-case'' binning scenario for skewed data is worse than for symmetric data.

<<SpatialLossCompare,echo=F,include=T,eval=T,fig.width=8, fig.height=3.5, out.width='.8\\linewidth', fig.pos='h',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="Lineplots for proportion of potential spatial loss over a range of bin sizes for standard and random binning of the fine version of the simulated data from each bivariate distribution.">>= 
spatlossdat <- read.csv("SpatialLossData.csv",header=TRUE)
# use all the square bins to plot bin width against proportion of max spataial loss
spatlossdatSquareBins <- spatlossdat[spatlossdat$widthx==spatlossdat$widthy,]
qplot(widthx,propSpatialLoss,geom="path",colour=bintype, linetype=bintype,
  ...
  theme_bw() + ylab("Prop. of Potential Spatial Loss") +
  xlab("Width/Height of Square Bins")
@

All rectangular binning algorithms require the specification of a bin width for each dimension and a binning origin. Smaller binwidths coincide with smaller spatial losses, but reducing bin sizes comes with a few important caveats. With smaller bins comes more computation requirements; a non-negligable consideration in settings with truly massive data. Additionally, the binwidth should be an integer multiple of the resolution of the data to avoid undesirable visual attributes in the binned scatterplot.

Consider the simulated uniform data, we have the fine version and the coarse version, which rounded to nearest even integer. Figure~\ref{fig:UniformStripes} displays the binned scatterplots under several scenarios. 

<<UniformStripes, echo=F,include=T,eval=T,fig.width=5, fig.height=5, out.width='.7\\linewidth', fig.pos='h',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="Binned Scatterplots of Uniform Data with 5X5 Bins">>= 
...
allbindat <- rbind(binoutfine[[1]],binoutcoarse[[1]],
                   binoutfineRand[[1]],binoutcoarseRand[[1]])
allbindat$SimDataType <- factor(rep(c("Fine","Coarse","Fine","Coarse"), 
                             each=nrow(binoutfine[[1]])),
                             levels=c("Fine","Coarse"))
allbindat$BinningType <- factor(rep(c("Standard","Random"),
                             each=2*nrow(binoutfine[[1]])),
                             levels=c("Standard","Random"))

qplot(binxs, binys, geom="tile", fill=binfreq, data=allbindat) +
  facet_grid(BinningType ~ SimDataType)+
  ggtitle(" ") +  
...
\ktm{
\begin{itemize}
\item Binning Algorithm: total and net spatial losses are always larger for random binning than standard (analytical proof)
\item Origin: 
  \begin{itemize}
    \item Doesn't much matter for fine resolution data but it does for coarse data.
    \item If recorded to resolution $\alpha$, origin at $x_{(1)} - \alpha/2$ is optimal (analytical proof then demo) Proof in Appendix~\ref{proof:offset}.
  \end{itemize} 
\item Binwidth: 
  \begin{itemize}
    \item generally smaller $\omega \rightarrow$ smaller $L^s$. 
    \item for fine data any binwidth is fine
    \item for coarse data need binwidths $\omega \ge \alpha$ (show plot with whitespace gaps)
    \item for coarse data binwidths should be scalar multiple of $\alpha$ (show plot with strips)
...
\subsection{Frequency binning choices}

\ktm{
\begin{itemize}
\item Freq Binning Type
  \begin{itemize}
    \item Comparison of standard, log and quantile binning
  \end{itemize}
\end{itemize}
...
\item Number of Colors:
  \begin{itemize}
    \item Perception driven limitation to 7 or less color bins
    \item As number colors increases, freq loss goes up?
  \end{itemize}
\end{itemize}
}
...
For this data example we will revisit the baseball data used earlier in this paper.  The characteristics of loss described in the discussion above are exemplified by viewing the loss for the baseball data in Figure ~\ref{fig:LossComparison1}. We find that the visual and numerical loss are in fact higher while using the random binning algorithm, but that the two binning algorithms are almost indistinguishable in terms of frequency loss.  
...
%----------------------------------------------------------------------------
\subsection{Optimal Offset for Uniform Data Recorded to Resolution $\alpha$}\label{proof:offset}
...
\noindent Spatial Loss, $L^S = \sum_{i=1}^{k} ||x_i-b_(x_i)|| $ is definitional minimized when $b_(x_i)$ is the \textit{geometric median}. The geometric median for $x_1, \dots, x_k = Q_x(.5) = (x_{\lceil\frac{k+1}{2}\rceil}+x_{\lfloor \frac{k+1}{2}\rfloor})/2$ , where $Q_x(\cdot)$ is the empirical quantile function. \\
...
If the same conditions are extended to the two dimensional case, then the origin for minimal spatial loss is at $(x_{(1)}-\alpha_x/2, y_{(1)}-\alpha_y/2)$ where $\alpha_x$ and $\alpha_y$  are the data resolution for each dimesion, respectively. 
