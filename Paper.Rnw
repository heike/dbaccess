...
Binning is a two-step procedure: we first assign each data pairs $(x, y)$ to a bin center $(x^\ast,y^\ast)$, and in a second step we find the frequency of each bin center, resulting in  triples  of the form $(x^\ast, y^\ast, c)$, where $c$ is the number of all observations assigned to bin center $(x^\ast,y^\ast)$.
...
We will first define a binning algorithm that results in observed values being allocated to equal sized bins in a deterministic manner. We then define a set of bin centers $\{ x_j^\ast \}$, with $1 \le j \le N_X$, where $N_X$ is the overall number of bins in dimension $X$.  To define the set of bin centers we need only specify the center of the first bin, $x^\ast_1$ and the binwidths, $\omega_X$.  Mathematically, binning corresponds to a function $b_X(.)$ that assigns to a value $x_i$ the closest bin center $x^\ast_j$ for some bin $j \in \{1, ..., N_X\}$, i.e. the binning function   $b_X(.) : x_i \rightarrow x^\ast_j$ is defined as 
% \begin{center}
...
Suppose that we have values $c_j$ for $j \in \{1,2,...J\}$ that we wish to divide into quantile based bins. The number of quantile based bins, $N_q$ , must be defined in order to assign values to bin centers. The quantile binning function $b^q_C(\cdot)$ assigns ordered values $c_{(j)}$ to bin centers $c^\ast_{(j)}$ as 
%
\begin{eqnarray}\label{quantbin}
c^\ast_{(j)} = b^q_C(c_{(j)}) = c_{(\eta)} \text{, where } \eta = \left\lfloor j/ \left\lceil J/N_c \right\rceil \right\rfloor \left\lceil J/N_c \right\rceil ,
\end{eqnarray}
%
\noindent and $\left\lceil \cdot \right\rceil$ is the ceiling function that rounds any real number up to the next highest integer.  To better understand this function we notice that $\left\lceil J/N_c \right\rceil$ represents the number of values to be assigned to each quantile bin, $n_q$, rounded up; thus $\eta$ is the index for the ordered value where the next lower quantile based bin will be centered.
