...
\ktm{ Generalized scatterplots \citep{GenScat} have also been introduced, which display all individual observations but use distortion of the point locations and axes to avoid overlapping points. This approach preserves fine data structure and allows color to be reserved for a third variable instead of frequency; however, the distortion warps the spatial information for the primary locations variables.  A extension of generalized scatterplots uses clustering and local principal components to allow ellipsoid oriented distortion to display local correlation structure in the data \citep{EllipseScatter}.}
