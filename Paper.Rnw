...
\hh{XXX ok, I don't quite agree with the description of frequency loss here. (a) the rendering on a continuous color scheme IS precise. but on a cognitive level we are not able to read the rendered values at the same precision. This effectively intorduces a second stage of binning ... then we are on the same page again.}
\end{itemize}
}

\hh{XXX what are you referring to by `it' in the next sentence?}
...
\ktm{The net spatial loss is a euclidean distance, but the units affiliated with this distance are based on the units on which each variable is recorded. \hh{XXX isn't this true for spatial loss in general and not just the net spatial loss? }  If the two variables in the binned scatterplot share the same units this leads to direct interpretability of the spatial loss.  However, if the two variables do not share the same units or magnitude of values it is advisable to standardize the variables prior to binning, thus the spatial loss is more universally interpretable as a distance in units of standard deviations. }
...
If we view the binned scatterplot as a visual estimator of the bivariate density then we may consider a few desirable properties of estimators: unbiasedness, consistency and efficiency. \ktm{As bin size becomes increasingly fine, the display will place the visual emphasis for the frequency information on more precise spatial locations but \hh{XXX what do you mean by bin density?} bin density have higher sampling variability due to the smaller number of observation per bin.} Choosing bin size for a binned scatterplot carries a trade-off between unbiasedness and consistency of the visual density estimate. For efficiency of density estimation, it may be tempting to claim that a minimally binning, with only one unique coordinate pair per bin, is an efficient estimator because it minimizes spatial loss. Minimally binned scatterplots are \textit{spatially} efficient, but density estimation requires the combination of spatial and frequency information, each of which have separate loss functions. Based on the conflicting effects on the estimation properties, of bin size can be seen as a smoother for density estimate that can be adjusted to cut through the noise in the data, but as with all data smoothers caution should be used so that fine signal is not destroyed.  
