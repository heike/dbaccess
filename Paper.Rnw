...
d1.ext <- xtd.data(d1)
@
<<scatterplots, dependson='setup', echo=F, results='hide'>>=
...
qplot(data=d1.hex, x=G, y=SO, geom="point", size=Freq) + 
  scale_area("Frequency", range=c(1, 4), trans="log", breaks=c(1, 10, 100, 1000, 10000))+ theme_bw() +
...
<<binningDemo,echo=F, results='none', cache=T>>=
      library(mvtnorm)
      set.seed(33)
...
<<RandomBinDemo,echo=F,include=FALSE,results='hide',  cache=TRUE>>=
nl <- ncol(d.rand)
dnew <- d.rand
binning <- c(10, 10)
for(x in 1:(nl)){
...
\begin{figure}[hbtp]
\centering
<<FrequencyCategories,echo=FALSE, out.width='\\textwidth', fig.width=8.5, fig.height=11>>=
...
computeLoss <- function(binning, data){
  ...
  datplot <- rbind(cbind(lRandom$NewData, Algorithm="Random"), cbind(lMean$NewData, Algorithm="Standard"))
    ...
    ggsave(paste("./figure/RandomStd", binning[1], binning[2], ".pdf", sep=""), width=8, height=5, units="in", dpi=1200)
#     qplot(G, SO, fill = log10(Freq), geom="tile", data = lMean$NewData, 
#       main=paste("Standard Binning (", binning[1], ", ", binning[2],")", sep="")) + 
#       scale_fill_gradient(low="#56B1F7", high="#132B43", guide="none", trans="log") + theme_bw()
#     ggsave(paste("./figure/Standard", binning[1], binning[2], ".pdf", sep=""), width=4.5, height=4.5, units="in")
  return(rbind(
    cbind(binX = binning[1], binY = binning[2], Function = "Random", lRandom$LossAll), 
    cbind(binX = binning[1], binY = binning[2], Function = "Standard", lRandom$LossAll)))
}
