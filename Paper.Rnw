...
Whether analytical or empirical, data is needed to demonstrate how loss is impacted by binning choices.

\hh{Just list the three different scenarios:
\begin{itemize}
\item {\bf Scenario I:} $x \sim \text{Exp}_\lambda, y \sim \text{Exp}_\lambda$ with $\lambda = 11$
\end{itemize}}
Data sets were simulated from bivariate distributions to be used throughout this section: Exponential, Normal and Uniform.  These distributions were selected for their variety in terms of shape, center and spread. 100,000 observation pairs were simulated for each set of simulated data, and the $x$ and $y$ values were drawn independently from univariate distributions; Exponential(11), Normal(50,$11^2$) and Uniform(0,100), respectively.  The parameters were selected to have data values roughly span the region $[0,100]^2$. The simulated data can be found in the top row of Figure~\ref{fig:SimDataScatterplots}. 


<<SimDataScatterplots,echo=F,include=T,eval=T,fig.width=8, fig.height=5.5, out.width='.8\\linewidth', fig.pos='h',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="Scatterplots of fine and coarse versions of the simulated bivariate data.">>= 
...
These simulated data sets are from continuous distributions, and thus the values are recorded to many decimal places. Real data is recorded to only the number of digits that the measurements allow; in many cases rounded to the nearest whole dollar or five minutes or two miles. The data resolution can be described by increment between data values. To observe binning loss under more realistic conditions, we created three data sets by rounding the values from the originally simulated data sets to the nearest even number, thus a data resolution of 2 units. This coarse version of the original simulated data is displayed in the bottom row of Figure~\ref{fig:SimDataScatterplots}. By exploring the loss properties for both the \textit{fine} and \textit{coarse} versions of the data, we identify which binning options are robust to the resolution to which data is recorded. 
...
In practice the $\alpha/2$ origin offset is found to be a reasonable choice for minimizing spatial loss for symmetric bivariate data at resolution $\alpha$ while using bin dimesions that are integer multiples of $\alpha$. Figure~\ref{fig:OriginOffsetSimulations} shows how the net spatial loss changes as the origin is slide from the minimum value to half the binwidth below the minimum value using the coarse simulated data with a resolution of 2 units. For the two symmetrically distributed data sets, normal and uniform, the origin offset of 1 unit below the minimum data value was found to minimize the net spatial loss for each considered binsize (2, 4, 6, 8, and 10). For the bivariately skewed exponential data, the 1 unit origin offset minimized net loss for the smaller intervals but was not optimal for the larger intervals.
