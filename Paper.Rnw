...
@
  \end{minipage}
\hfil\begin{minipage}{.695\linewidth}
...
binoutQuantLogFreqGroups <- freqBin(binout1, binType="quantile", ncolor=5, logCount=TRUE)
...
p5 <- qplot(binxs, binys, geom="tile", fill=freqgroup,
...
@
\end{minipage}
\caption{\label{freqbinning}Simulated bivariate normal data spatially binned using standard algorithm with 5X5 unit bins and origin at (0,0). Binned Scatterplot with raw frequencies (left) and binned scatterplots with five frequency bins (right)."}
\end{figure}
