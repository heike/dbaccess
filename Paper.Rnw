...
These simulated data sets are from continuous distributions, and thus the values are recorded to many decimal places \hh{XXX how many exactly? . Real data is recorded to only the number of digits that the measurements allow, \hh{and} in many cases rounded \hh{even further. }% to the nearest whole dollar or five minutes or two miles.
\hh{XXX this is a definition of data resolution, make that clear.} The data resolution can be described by increment between data values. To observe \hh{loss from} binning  under more realistic conditions, we create three data sets by rounding the values from the originally simulated data sets to the nearest even number, thus a data resolution of 2 units. This coarse version of the original simulated data is displayed in the bottom row of Figure~\ref{fig:SimDataScatterplots}. By exploring the loss properties for both the \textit{fine} and \textit{coarse} versions of the data, we identify which binning options are robust to the resolution to which data is recorded. 
