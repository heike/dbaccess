...
\ktm{Log transforming the frequencies prior to binning and using quantile based binning on the raw frequencies are two methods of trying to fix the same problem: heavily right skewed bin counts. However, using both -- quantile log frequency binning -- is not a recommended approach. The frequency groups for quantile binning are invariant to the log transformation because a monotone one-to-one transformation will not alter groupings based on quantiles. In the common situation where bin frequencies are heavily skewed, the choice of frequency binning is between between quantile frequency binning and standard log frequency binning.}
