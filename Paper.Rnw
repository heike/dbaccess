...
Figure~\ref{fig:StandardFreqBinning4710none} displays several spatially binned scatterplots. On the right frequency is mapped continuously to colour; on the left  there is a varying numbers of  standard binned frequencies. It is trivial to discern differences between similarly shaded tiles
when there are only four shades. This becomes more difficult -- but still possible -- at seven frequency bins, but at ten frequency bins we are hardly able to discriminate between colours better than in the continuous shading. 
...
Frequency binning using log counts exhibits a very similar behaviour to binning of untransformed frequencies.
...
%However, using both -- quantile log frequency binning -- is not a recommended approach \hh{XXX not recommended is too strong. It actually does not help to do both, as can be seen in the figure}. 
...
\hh{XXX fig 13 starts a new thought: }
...
\hh{XXX What conclusion do you want to draw from figure 13?}
