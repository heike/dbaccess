...
p4 <- qplot(binxs, binys, geom="tile", fill=freqgroup, data=binoutLogFreqGroups[[1]],
    ...
    xlab("") + ylab("")+
  scale_fill_gradient("Log Frequency", low="#56B1F7", high="#132B43", guide = guide_legend(label.position="bottom", label.hjust=0.5,  title.position = "top"), breaks=unique(binoutLogFreqGroups[[1]]$freqgroup)) + 
  theme_bw() + theme(legend.position="bottom",aspect.ratio=1,
                     legend.key.width = unit(1, "cm"),
                     panel.grid= element_blank()) 
...
p6 <-qplot(ncolor, totalFreqLoss, geom="path",linetype=FreqBinType,
      data=plotdat , color = FreqBinType, size=I(1)) +
  facet_grid(.~DistType, scales="free_y") +  theme_bw() +
  scale_x_continuous(breaks=1:10) + ylim(0,40) +
  xlab("Freq Bins") + ylab("Total Log Freq Loss") + 
  theme(legend.position="bottom")
