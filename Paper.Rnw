...
Figure~\ref{fig:StandardFreqBinning4710none} displays several spatially binned scatterplots. On the left frequency is mapped continuously to colour; on the right  there is a varying numbers of  standard binned frequencies. It is trivial to discern differences between similarly shaded tiles
