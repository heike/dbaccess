...
\ktm{If we view the binned scatterplot as a visual estimator of the bivariate density then we may consider a few desirable properties of estimators: unbiasness, consistency and efficiency. As binsize becomes increasingly fine, the display will place the visual emphasis for the frequency information on more precise spatial locations but bin density have higher sampling variability due to the smaller number of observation per bin. Choosing bin size for a binned scatterplot carries a trade-off between unbiasness and consistency of the visual density estimate. For efficiency of density estimation, it may be tempting to claim that a minimally binning, with only one unique coordinate pair per bin, is an efficient estimator because it minimizes spatial loss. Minimally binned scatterplots are \textit{spatially} efficient, but density estimation requires the combination of spatial and frequency information, each of which have seperate loss functions. Based on the conflicting effects on the estimation properties, of bin size can be seen as a smoother for density estimate that can be adjusted to cut through the noise in the data, but as with all data smoothers caution should be used so that fine signal is not destroyed.  }
...
For data with a coarse resolution the location of the origin is also important because the origin controls the proximity of possible data values to bin centers. \ktm{We define the \textit{origin offset} for each dimension, ($o_x$, $o_y$), such that 
%
\begin{eqnarray}\label{OriginOffset}
(\beta_{0,x},\beta_{0,y}) = (x_{(1)}, y_{(1)}) - (o_x, o_y),
\end{eqnarray}  
%
where $(\beta_{0,x},\beta_{0,y})$ is the bivariate bin origin, $x_{(1)}$ and $y_{(1)}$ are the minimum data value for each dimension, and $(o_x, o_y)$ is the origin offset. Thus the offset indicates the number of units in each dimension to shift the origin below the highest possible origin. }

\ktm{It can be shown analytically that an origin offset of ($\alpha_x/2$,$\alpha_y/2$) units will minimize the net spatial loss in the situation with the following three properties: (i) data are recorded to a resolution of $\alpha_x$ units in the X dimension and $\alpha_y$ units in the Y dimension, (ii) points are symmetric distributed within rectangular bins, (iii) the bin dimensions are integer multiples of $\alpha_x$ and $\alpha_y$, respectively (see proof in Appendix~\ref{proof:offset}). }
...
\ktm{Figure~\ref{fig:OriginOffsetSimulations} shows how the net spatial loss changes as the origin offset is changed while binning the coarse simulated data using the standard rectangular binning algorithm. Note that for simplicity, only origin offsets for with equal offset in each dimension are included in this simulation. Since the data resolution is two units in each dimension, we will pay special attention to an origin offset of (1,1) in order to assess how well the theoretically optimal origin offset at ($\alpha_x/2$,$\alpha_y/2$) works for data that violate the assumptions to differing degrees. The round glyphs indicate the origin offset where the net loss reached an absolute minimum. For the two symmetrically distributed data sets, normal and uniform, the origin offset of one unit was found to either minimize the net spatial loss or achieve a local minimum very near to the overall minimum (within a 0.2\% increase from the minimum spatial loss) for each considered bin size.} %\hh{XXX - no! the value at -1 is close, but the dots are in the actual minimal values. This is only a single example - the expected minimum might still be in -1. }
