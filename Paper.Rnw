...
x^\ast = b(x) =  b_X \cdot \left\lfloor (x-x_1^\ast)/ b_X + 1/2\right\rfloor + x_1^\ast
\end{eqnarray*}
where $x_{(1)}$ is the center of the first bin and $\lfloor . \rfloor$ denotes the floor function, $\lfloor x \rfloor$ is the largest integer value smaller than $x$. An intuitive understanding of the formula is developed by noting that $(x-x_1^\ast)/ b_X$ is the number of bin widths that $x$ is above $x_1^\ast$ and that $\frac{b_X/2}{b_X}=\frac{1}{2}$ is the number of bin widths that $x_1^\ast$ is above $x_{(1)}$. The value $\lfloor (x-x_1^\ast)/ b_X + 1/2\rfloor$ then represents the number of bins centers that lie between $x_{(1)}$ and $x$.  Thus, the binning function is simply finding the location of the bin center for $x$ by adding the distance between the bin centers for $x_{(1)}$ and $x$ to the known bin center for $x_{(1)}$.
...
%   {./figure/RandomBinningDataPoints1}}
  ...
  \caption{In illustration (a) points are assigned to bins centered at (5, 25) an (15,25). Points in the standard algorithm are all contained within the bin region. Illustration (b) displays the points assigned to a single bin along with key bin attributes: the visual bin center, ($x_1^\ast$, $y_1^\ast$), and the numerical bin center, ($\bar{x}$,$\bar{y}$). }
