...
qplot(data=d1.hex, x=G, y=SO, geom="point", size=Freq) + 
  scale_area("Frequency", range=c(1, 4), trans="log", breaks=c(1, 10, 100, 1000, 10000))+ theme_bw() + xlab("Games (Count)")+ ylab("Strike Outs (Count)") + theme(legend.position="bottom")
...
%   {./figure/RandomBinningDataPoints1}}
  \subfloat[Bin Attribute Diagram]{\includegraphics[keepaspectratio=TRUE, width=4in]
  {./figure/Binning-DataVisualNumericalCenter.pdf}}
