...
\begin{figure}[hbtp]
  ...
  \subfloat[Hexagonal binned bubble plot]{\includegraphics[keepaspectratio=TRUE,width=.48\textwidth]{./figure/BinScatter.png}} 
	...
	\caption[Traditional and adapted scatterplots for games vs. strikeouts data.]{\label{fig:scatter-alpha}Traditional and adapted scatterplots for games vs. strikeouts data. Note improvement in conveying visual density information in the aggregation based plots on the bottom row.}
\end{figure}
